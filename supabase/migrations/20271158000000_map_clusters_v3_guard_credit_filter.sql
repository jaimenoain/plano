-- Backend performance: skip the per-row credit-filter call on the no-filter map pan
-- (forensic backend-latency audit, 2026-06-22)
--
-- get_map_clusters_v3 is the single highest-frequency RPC in the app — it fires
-- on every (debounced) map pan/zoom. Its WHERE clause calls the SECURITY DEFINER
-- function public.building_matches_credit_filters(b.id, …) once per candidate
-- building in the viewport. That function returns TRUE (i.e. "no credit filter,
-- keep the row") whenever:
--
--     p_company_id IS NULL AND (p_roles IS NULL OR cardinality(p_roles) = 0)
--
-- which is the overwhelmingly common case (no company/role credit filter set).
-- Yet the function is still invoked per row, paying call + planning overhead on
-- thousands of buildings every pan for a result that is trivially TRUE.
--
-- Fix: guard the call with the function's own no-filter condition, ORed in front
-- of it. Because PostgreSQL short-circuits OR left-to-right and the guard depends
-- only on the (row-invariant) PL/pgSQL filter variables, the function is now
-- called ONLY when a credit filter is actually present.
--
-- PROVABLY OUTPUT-EQUIVALENT (this is the whole point — no result can change):
--   • No filter set  → guard = TRUE  → predicate TRUE, function never called.
--                       Original: function returns TRUE. Identical.
--   • Filter set     → guard = FALSE → predicate = function(b.id, …).
--                       Original: same function call. Identical.
-- The guard mirrors building_matches_credit_filters' exact internal short-circuit
-- (defined in 20270836000000), so the two are equal by construction.
--
-- The function body below is otherwise a VERBATIM copy of the definitive version
-- from 20271146000000 (gap-photo-count fix) — only the single
-- building_matches_credit_filters predicate line is wrapped. Signature, columns,
-- LATERAL award lookup (which populates the returned winner_award_name column and
-- therefore must stay unconditional), GRANT and COMMENT are unchanged.
--
-- The same one-line guard applies verbatim to get_map_clusters / _v2,
-- get_buildings_list and search_buildings_v2 (all call the function per row);
-- left for a follow-up to keep this migration's blast radius to the hottest RPC.
--
-- NEEDS APPLY in the Supabase SQL Editor (this project applies migrations by hand).
...
BEGIN
  ...
  WITH filtered_buildings AS (
    ...
    LEFT JOIN (
      -- Count only public photos: review_images.review_id → building_posts.id
      -- (FK re-pointed in migration 20270872000000). Contacts-only and private
      -- posts are excluded so "No photos" means no publicly visible photos.
    ...
    WHERE (b.is_deleted IS FALSE OR b.is_deleted IS NULL)
      ...
      -- Guarded credit filter: only call the per-row SECURITY DEFINER function
      -- when a company/role filter is actually set. The first branch is the
      -- function's own no-filter short-circuit (20270836000000), so this is
      -- output-equivalent — see this migration's header.
      AND (
...
'Browse-only map cluster RPC (Phase 3 + Photography Gaps + Century filter). photos_count counts only public building_posts (visibility = public or NULL). Supports both inclusion (construction_statuses) and exclusion (exclude_construction_statuses) status filters. Credit filter is guarded to skip the per-row function call when no company/role filter is set (20271158000000).';
