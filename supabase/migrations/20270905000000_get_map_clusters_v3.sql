-- Phase 3 — Slim Browse mode: get_map_clusters_v3
--
-- Design:
--   * Browse-only RPC. No `query` text-search code path — Find mode owns text
--     search via search_buildings_v2 (Phase 1) + useUnifiedSearch (Phase 2).
--   * NO SILENT EXCLUSIONS:
--     - removes the v2 hardcoded `Demolished | Lost | Under Construction | Unbuilt`
--       default exclusion. Construction-status filter is honoured if the caller
--       supplies one; otherwise all statuses are returned.
--     - removes the `popularity_score >= -50` floor.
--   * Same return shape as v2 so MapMarkers / pin styles need no changes.
--   * `is_deleted = false` is the only structural default.
--   * `Unbuilt` rows have `location IS NULL` in practice; a `location IS NOT NULL`
--     guard is kept so the spatial join is safe.
--
-- Default status behaviour is owned by the caller. The "Show demolished" toggle
-- in FilterDrawer flips between sensible default sets — that logic lives in
-- useMapData, not here.
...
-- v2 and v1 are intentionally NOT dropped. Post-rebuild cleanup task (after v3
-- has been live for a release cycle) will drop them. Until then they remain
-- callable so rollback is mechanical.
...
'Browse-only map cluster RPC (Phase 3). No text search (Find mode owns that). No silent status / popularity exclusions — all defaults are caller-driven via filter_criteria.';
