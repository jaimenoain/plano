-- get_buildings_list: the viewport bbox always applies, and hidden buildings
-- are excluded — so the SERP list and the map pins answer the same question.
--
-- Roadmap Task 4.1. Two membership divergences from get_map_clusters_v3:
--
--   1. The bbox predicate was short-circuited to TRUE whenever
--      filter_criteria->>'query' was non-empty (added by
--      20270509000000_update_get_buildings_list_search_global.sql). The list
--      then went GLOBAL while the pins stayed viewport-bound and ignored the
--      query entirely — the two panes answering completely different
--      questions. Global text search is search_buildings_v2's job (/map Find
--      mode), which pushes its own pins, so the panes agree there by
--      construction. Both callers of this function — the /map browse list and
--      the collection Discover list — are viewport lists.
--
--   2. get_map_clusters_v3 excludes ub.status = 'ignored'; this did not. A
--      building the user hid stayed in the list forever while never drawing a
--      pin. Owner decision (2026-08-06): hide it from the list too.
--      The 'none' branch of the status filter keeps matching ub.status IS NULL.
--
-- Body is otherwise byte-identical to the live definition, which is the one in
-- 20271175000000_search_buildings_v2_authoritative_name_search.sql (applied
-- 2026-07-30, later than 20271174000000 and carrying its b.id tiebreaker).
-- Signature and RETURNS TABLE unchanged, so CREATE OR REPLACE is sufficient.
--
