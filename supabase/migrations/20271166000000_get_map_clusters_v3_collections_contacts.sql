-- get_map_clusters_v3: wire Folders/Collections and Curators & friends into the
-- map pins via the shared helpers, so the map surface matches the SERP list.
-- Guarded no-op when the filters are unset. Everything else is verbatim from the
-- prior definition (20271158000000 guard-credit-filter version).
...
CREATE OR REPLACE FUNCTION public.get_map_clusters_v3(min_lat double precision, max_lat double precision, min_lng double precision, max_lng double precision, zoom_level double precision, filter_criteria jsonb DEFAULT NULL::jsonb)
 RETURNS TABLE(id text, lat double precision, lng double precision, count bigint, is_cluster boolean, status text, name text, slug text, image_url text, popularity_score integer, tier_rank text, max_tier integer, winner_award_name text, photos_count integer, city text)
 LANGUAGE plpgsql
 SECURITY DEFINER
