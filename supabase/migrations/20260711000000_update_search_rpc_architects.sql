-- Update search_buildings RPC to support architects filtering
...
CREATE OR REPLACE FUNCTION search_buildings(
  ...
  filters jsonb DEFAULT NULL, -- {cities: text[], country: text, styles: text[], architects: text[]}
...
BEGIN
  ...
  IF filters IS NOT NULL THEN
    ...
    IF filters ? 'architects' AND jsonb_typeof(filters->'architects') = 'array' THEN
        SELECT ARRAY(SELECT jsonb_array_elements_text(filters->'architects')) INTO v_architects_filter;
    END IF;
  END IF;
  ...
  WHERE
    ...
    AND
    (
        v_architects_filter IS NULL
        OR cardinality(v_architects_filter) = 0
        OR (ba_agg.architects_names IS NOT NULL AND ba_agg.architects_names && v_architects_filter)
    )
    AND
    (
