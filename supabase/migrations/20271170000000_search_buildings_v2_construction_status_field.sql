-- search_buildings_v2 (Find mode): also RETURN the raw construction status
-- (buildings.status) as `construction_status`, so Find-mode results carry the
-- same Lost / Unbuilt / Under Construction / Temporary signal as Browse. Builds
-- verbatim on the collections/contacts version; only the RETURNS TABLE signature
-- and one projected column change.
...
DROP FUNCTION IF EXISTS public.search_buildings_v2(text, integer, integer, jsonb);
