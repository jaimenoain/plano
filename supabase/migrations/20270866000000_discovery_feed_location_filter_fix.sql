-- Explore location filter: match country via ISO code (buildings.country_code),
-- fall back to substring country name when code missing on row; align region
-- filter with city filter (case-insensitive substring in address or city).
...
GRANT EXECUTE ON FUNCTION public.get_discovery_feed(uuid, int, int, text, uuid, uuid[], uuid[], uuid[], text, text, text[], uuid[], text[], text) TO anon;
GRANT EXECUTE ON FUNCTION public.get_discovery_feed(uuid, int, int, text, uuid, uuid[], uuid[], uuid[], text, text, text[], uuid[], text[], text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_discovery_feed(uuid, int, int, text, uuid, uuid[], uuid[], uuid[], text, text, text[], uuid[], text[], text) TO service_role;
