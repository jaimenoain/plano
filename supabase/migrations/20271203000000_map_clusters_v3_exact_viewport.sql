-- get_map_clusters_v3: aggregate over the EXACT viewport, not an inflated box.
--
-- Roadmap Task 4.1 — "the SERP list shows exactly the items within the visible
-- map viewport". The map bbox was being inflated twice: the client padded 30%
-- per side (useMapData.calculateFetchBox) and this function padded a further
-- 10% of the span per side, so the pins aggregated ~1.9x the linear span
-- (~3.7x the area) the SERP list paged over. A cluster bubble sitting inside
-- the viewport could therefore count buildings the list could never show.
--
-- Fix: v_safe_* clamp only. The client buffer goes to 0 in the same PR, so at
-- rest both panes query the identical box and a cluster count is exactly the
-- number of rows get_buildings_list can page through. The prefetch buffer is
-- not missed: the cluster query keeps `placeholderData: keepPreviousData`, so
-- the previous pins stay painted during a refetch rather than blanking.
--
-- Body is otherwise byte-identical to 20271172000000_five_rank_map_tiers.sql.
-- Signature and RETURNS TABLE unchanged, so CREATE OR REPLACE is sufficient —
-- no DROP (a DROP+CREATE that changed the arg list would leave two live
-- overloads and make the RPC ambiguous to PostgREST; see docs/AI_STATUS.md).
--
...
-- Both roles are then granted back on purpose: this is the public map surface: anonymous visitors browse pins before signing in.
