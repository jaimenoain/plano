...
-- We use "ON COMMIT DROP" so it cleans up after transaction, but "IF NOT EXISTS" prevents errors if called multiple times in session.
...
BEGIN
    ...
    SELECT jsonb_build_object(
        'optimist', (SELECT jsonb_agg(jsonb_build_object('name', username, 'avatar', avatar_url, 'value', avg_rating, 'id', user_id)) FROM (SELECT * FROM member_aggs ORDER BY avg_rating DESC LIMIT 3) t),
        'hater', (SELECT jsonb_agg(jsonb_build_object('name', username, 'avatar', avatar_url, 'value', avg_rating, 'id', user_id)) FROM (SELECT * FROM member_aggs ORDER BY avg_rating ASC LIMIT 3) t),
        'prolific', (SELECT jsonb_agg(jsonb_build_object('name', username, 'avatar', avatar_url, 'value', count, 'id', user_id)) FROM (SELECT * FROM member_aggs ORDER BY count DESC LIMIT 3) t),
        'wordsmith', (SELECT jsonb_agg(jsonb_build_object('name', username, 'avatar', avatar_url, 'value', avg_length, 'id', user_id)) FROM (SELECT * FROM member_aggs WHERE review_count >= 3 ORDER BY avg_length DESC LIMIT 3) t),
        'novelist', (SELECT jsonb_agg(jsonb_build_object('name', username, 'avatar', avatar_url, 'value', total_words, 'id', user_id)) FROM (SELECT * FROM member_aggs ORDER BY total_words DESC LIMIT 3) t),
        'quickestDraw', (SELECT jsonb_agg(jsonb_build_object('name', m.username, 'avatar', m.avatar_url, 'value', q.avg_delay_hours, 'id', m.user_id)) FROM (SELECT * FROM quickest_draw_calc ORDER BY avg_delay_hours ASC LIMIT 3) q JOIN member_aggs m ON m.user_id = q.user_id),
        'contrarian', (SELECT jsonb_agg(jsonb_build_object('name', m.username, 'avatar', m.avatar_url, 'value', c.avg_deviation, 'id', m.user_id)) FROM (SELECT * FROM contrarian_calc ORDER BY avg_deviation DESC LIMIT 3) c JOIN member_aggs m ON m.user_id = c.user_id),
        'timeTraveler', (SELECT jsonb_agg(jsonb_build_object('name', m.username, 'avatar', m.avatar_url, 'value', t.avg_gap_years, 'id', m.user_id)) FROM (SELECT * FROM time_traveler_calc ORDER BY avg_gap_years DESC LIMIT 3) t JOIN member_aggs m ON m.user_id = t.user_id),
        'completist', (SELECT jsonb_agg(jsonb_build_object('name', m.username, 'avatar', m.avatar_url, 'value', c.max_count, 'id', m.user_id, 'metadata', c.top_director)) FROM (SELECT * FROM completist_calc ORDER BY max_count DESC LIMIT 3) c JOIN member_aggs m ON m.user_id = c.user_id)
    ) INTO v_superlatives;
...
BEGIN
    ...
    -- 3. Prepare Logs
    -- NOTE: We used CREATE TEMP TABLE IF NOT EXISTS at top of file (or outside logic)
    -- Here we ensure it is clean and populated.

    -- Ensure table exists in current session if it wasn't created by the global block above (e.g. new session)
    CREATE TEMPORARY TABLE IF NOT EXISTS temp_group_logs (
        id uuid,
        user_id uuid,
        film_id text,
    ...
    -- Cleanup? ON COMMIT DROP handles it, but truncate is good practice if used again in session
    TRUNCATE temp_group_logs;
END;
$$;
