-- Add Notification Intelligence to Admin Dashboard Stats
-- Updates get_admin_dashboard_stats to include notification engagement and unread distribution
-- This moves the calculation from client-side (subject to RLS) to server-side (SECURITY DEFINER)
...
BEGIN
  ...
  BEGIN
    ...
    -- We want to see how many unread notifications active users have accumulated over time.
