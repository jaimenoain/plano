-- Audit Fixes: Logic Corrections for Group Stats Engine
-- 1. Fixed "Hidden Gems" logic: Removed redundant OR check and added NULL safety for session dates.
-- 2. Fixed "Radar Chart" logic: Used LEFT JOIN on target_genres to ensure all 8 axes are returned.
-- 3. Added statistical significance checks (min 3 reviews) to Superlatives to prevent "one-hit wonders".
-- 4. Fixed "Novelist" word count logic to correctly handle empty strings (returning 0 instead of 1).
-- 5. Fixed "Quickest Draw" logic: Normalized session dates to UTC Midnight to prevent timezone skew.
-- 6. Fixed "Contrarian" logic: Implemented "Leave-One-Out" calculation to remove self-bias from deviation stats.
-- 7. Fixed "Time Traveler" logic: Enforced strict check on `watched_at` to prevent backfill false positives.

-- Ensure temp table exists for function validation
