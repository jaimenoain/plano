-- 1. Add short_id and slug to buildings
ALTER TABLE public.buildings ADD COLUMN IF NOT EXISTS short_id SERIAL;
-- Ensure uniqueness for short_id (SERIAL usually implies sequence but we want unique constraint for lookups)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'buildings_short_id_key') THEN
    ALTER TABLE public.buildings ADD CONSTRAINT buildings_short_id_key UNIQUE (short_id);
  END IF;
END $$;
...
-- 2. Backfill slugs
-- Simple slugify: lowercase, remove non-alphanumeric (except space/dash), replace spaces with dashes
UPDATE public.buildings
SET slug = lower(regexp_replace(regexp_replace(name, '[^a-zA-Z0-9\s-]', '', 'g'), '\s+', '-', 'g'))
WHERE slug IS NULL;


-- 3. Update search_buildings RPC
