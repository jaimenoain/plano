-- Phase 3 fix — restore NULL-status buildings to the map.
--
-- Problem: `useMapData` sends `construction_statuses: ['Built','Temporary']`
-- as the Browse-mode default. v3's WHERE clause uses
-- `b.status::text = ANY(v_construction_statuses)`, which evaluates to NULL
-- (not TRUE) for rows whose `status` column is NULL. Those rows drop out of
-- the result set, so legacy buildings with no recorded construction status
-- never appear on the search-page map. (Spot check: London bbox went from
-- 1824 pins on v2 to 1334 on v3 — 490 NULL-status buildings missing.)
--
-- Fix: add an optional `exclude_construction_statuses` filter alongside the
-- existing inclusion filter. The default Browse path (and the "Show
-- demolished" toggle) switch to passing an exclusion list, which naturally
-- lets NULL pass through (`NULL <> ALL(...)` is NULL, but we OR in a NULL
-- guard). Explicit checkbox picks continue to use the inclusion list and
-- keep their strict semantics.
--
-- Function signature is unchanged — only the parsing of `filter_criteria`
-- and one WHERE-clause line grow. CREATE OR REPLACE is sufficient.
...
'Browse-only map cluster RPC (Phase 3). No text search (Find mode owns that). Two construction-status filters: inclusion `construction_statuses` is strict; exclusion `exclude_construction_statuses` lets NULL status pass through so legacy rows still render by default.';
