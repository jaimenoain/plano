-- Fix search_buildings RPC to use valid profile columns (username instead of first_name/last_name)
-- Migration ID: 20261115000000_fix_search_rpc_profiles.sql

-- Drop existing function first because we are changing the return type or signature might conflict if not handled by REPLACE (though here signature is same)
