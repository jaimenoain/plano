-- =============================================================================
-- Migration: discovery_feed_keyset_pagination
--
-- Root cause: get_discovery_feed excludes every building the caller already has a
--   user_buildings row for, and Explore writes such a row for each building the user
--   pages past. The result set therefore shrinks *while* the user browses it, so the
--   client's OFFSET-based paging (offset = pages * 10) landed past buildings that had
--   never been rendered. Users reported buildings silently missing from the feed.
--
-- Fix: add an optional keyset cursor matching the function's existing deterministic
--   order (save_count DESC, id ASC). With a cursor the next page starts exactly where
--   the last one ended, no matter how many rows dropped out in between.
--
--   p_offset is retained and still honoured when no cursor is supplied, so any client
--   that has not shipped the cursor yet keeps working unchanged.
--
-- The body is otherwise identical to 20271028000000_fix_discovery_feed_video_url.sql;
-- it is wrapped in a CTE so the cursor predicate can filter on the aggregated
-- save_count without repeating the aggregate.
-- =============================================================================

-- Adding parameters makes a NEW overload rather than replacing the old one, and two
-- live overloads make the RPC ambiguous to PostgREST (every call then 300s). Drop the
-- previous signature first — the same trap 20270863000000 had to clean up before.
DROP FUNCTION IF EXISTS public.get_discovery_feed(
  uuid, int, int, text, uuid, uuid[], uuid[], uuid[], text, text, text[], uuid[],
...
CREATE OR REPLACE FUNCTION public.get_discovery_feed(
  ...
  p_after_save_count bigint DEFAULT NULL,
...
BEGIN
  ...
  WITH candidates AS (
    ...
    HAVING
      ...
      OR EXISTS (
        SELECT 1 FROM building_posts bp
        WHERE bp.building_id = b.id AND bp.video_url IS NOT NULL
      )
  )
  SELECT
    c.id,
  ...
  FROM candidates c
  WHERE
    -- Keyset cursor. Written out rather than as a row comparison because the sort
    -- mixes directions (save_count DESC, id ASC), which row-wise `<` cannot express.
    p_after_id IS NULL
    OR c.save_count < p_after_save_count
    OR (c.save_count = p_after_save_count AND c.id > p_after_id)
  ORDER BY
    c.save_count DESC,
    c.id
  LIMIT
    p_limit
  -- Ignored once a cursor is supplied: the cursor already encodes the position, and
  -- applying both would skip a page's worth of buildings.
  OFFSET
    CASE WHEN p_after_id IS NULL THEN p_offset ELSE 0 END;
END;
$$;
...
COMMENT ON FUNCTION public.get_discovery_feed(
  ...
  'Explore discovery feed. Paginate with the (p_after_save_count, p_after_id) keyset cursor taken from the last row of the previous page; p_offset is legacy and only applies when no cursor is given.';
