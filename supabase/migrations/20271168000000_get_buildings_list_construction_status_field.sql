-- get_buildings_list: also RETURN the raw construction status (buildings.status)
-- as `construction_status`, so the SERP list can render a chip (Lost / Unbuilt /
-- Under Construction / Temporary). The existing `status` column stays the *user
-- library* status (visited/saved/none) — these are two different dimensions.
-- Builds verbatim on the Slice-2 collections/contacts version; only the RETURNS
-- TABLE signature and one projected column change.
...
DROP FUNCTION IF EXISTS public.get_buildings_list(double precision, double precision, double precision, double precision, jsonb, integer, integer);
