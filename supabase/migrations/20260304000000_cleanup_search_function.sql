...
-- Force drop the function with CASCADE to handle any dependent triggers or views (though there shouldn't be any for this RPC)
-- This signature matches the original one that likely caused the "Zombie" state.
DROP FUNCTION IF EXISTS search_films_tiered(TEXT, INTEGER[], TEXT[], INTEGER[], INTEGER, INTEGER, INTEGER, INTEGER) CASCADE;

-- Re-create the function with all fixes applied
...
BEGIN
    ...
    WITH tier1_candidates AS (
        ...
        SELECT DISTINCT ON (f.id)
            f.id::text, -- Explicit cast to text
            f.tmdb_id,
            f.title,
            f.original_title,
            f.poster_path,
        ...
        AND (p_countries IS NULL OR (
            -- Safety check: Ensure countries is not null before expanding
        ...
        AND (p_decade_starts IS NULL OR (
            EXISTS (
                ...
                WHERE EXTRACT(YEAR FROM NULLIF(f.release_date, '')::DATE) >= d
                AND EXTRACT(YEAR FROM NULLIF(f.release_date, '')::DATE) < d + 10
            )
        ))
        AND (p_runtime_min IS NULL OR f.runtime >= p_runtime_min)
        AND (p_runtime_max IS NULL OR f.runtime <= p_runtime_max)
        ORDER BY f.id -- [CRITICAL FIX] Required by DISTINCT ON
    ),
    tier2_candidates AS (
        -- Tier 2: Community (Everything else in DB matching filters)
        SELECT
            f.id::text, -- Explicit cast
            f.tmdb_id,
            f.title,
            f.original_title,
            f.poster_path,
            ...
            f.community_vote_average
        FROM films f
        -- Cast ID to text for comparison with tier1 results
        WHERE f.id::text NOT IN (SELECT id FROM tier1_candidates)
        -- Apply Filters
        AND (p_query IS NULL OR f.title ILIKE '%' || p_query || '%')
        AND (p_genre_ids IS NULL OR f.genre_ids && p_genre_ids)
        AND (p_countries IS NULL OR (
