DROP FUNCTION IF EXISTS search_films_tiered(TEXT, INTEGER[], TEXT[], INTEGER[], INTEGER, INTEGER, INTEGER, INTEGER);
...
BEGIN
    ...
    WITH tier1_candidates AS (
        ...
        SELECT DISTINCT ON (f.id)
            f.id::text, -- Explicit cast to text to match Return Type
            f.tmdb_id,
            f.title,
            f.original_title,
            f.poster_path,
