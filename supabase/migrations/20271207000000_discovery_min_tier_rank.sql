-- Task 5.7 — "Show All Buildings" filters: quality-tier ("min_tier_rank").
--
-- The collection map's discovery layer (Show All Buildings) had no filters at
-- all — filter_criteria was passed as `{}` on both RPCs it uses. Era filtering
-- (`centuries`) already worked; a quality-tier filter did not exist anywhere,
-- because `min_rating` only exposes three coarse bands (1/2/3) tied to the
-- rating-star UI, not the tier labels themselves, and it doubles as a
-- *personal*-rating-adjacent concept the discovery UI doesn't want to reuse.
--
-- This adds a new, independent key `min_tier_rank` (one of 'Top 1%' / 'Top 5%'
-- / 'Top 10%' / 'Top 20%') to both get_map_clusters_v3 and get_buildings_list,
-- matched against buildings.tier_rank with the same "at or above this band"
-- semantics min_rating already uses. Additive and optional: when the key is
-- absent (every existing caller), behaviour is byte-for-byte unchanged.
--
-- No new tables, no RLS change. Bodies otherwise byte-identical to
-- 20271203000000_map_clusters_v3_exact_viewport.sql and
-- 20271204000000_buildings_list_always_bbox.sql.
--
-- types-neutral: signature/RETURNS TABLE unchanged, so gen-types is a no-op
-- for the column list, but is still run to refresh the Functions Json args.
...
BEGIN
  ...
  -- Clamp only — NO inflation. The caller's box is the visible viewport and the
  -- SERP list pages over exactly that box, so any padding here silently counts
  -- buildings the list cannot show. The former `± span * 0.1` widening (and the
  -- v_lat_span / v_lng_span locals it needed) is gone.
  ...
  IF filter_criteria IS NOT NULL THEN
    ...
    IF filter_criteria ? 'min_tier_rank' AND filter_criteria->>'min_tier_rank' IS NOT NULL THEN
      v_min_tier_rank := filter_criteria->>'min_tier_rank';
      IF v_min_tier_rank NOT IN ('Top 1%', 'Top 5%', 'Top 10%', 'Top 20%') THEN
        v_min_tier_rank := NULL;
      END IF;
    END IF;
  ...
  WITH filtered_buildings AS (
    ...
    WHERE (b.is_deleted IS FALSE OR b.is_deleted IS NULL)
      ...
      AND (
        v_min_tier_rank IS NULL
        OR (v_min_tier_rank = 'Top 20%' AND b.tier_rank::text IN ('Top 20%', 'Top 10%', 'Top 5%', 'Top 1%'))
        OR (v_min_tier_rank = 'Top 10%' AND b.tier_rank::text IN ('Top 10%', 'Top 5%', 'Top 1%'))
        OR (v_min_tier_rank = 'Top 5%' AND b.tier_rank::text IN ('Top 5%', 'Top 1%'))
        OR (v_min_tier_rank = 'Top 1%' AND b.tier_rank::text = 'Top 1%')
      )
      AND (
...
REVOKE ALL ON FUNCTION public.get_map_clusters_v3(double precision, double precision, double precision, double precision, double precision, jsonb) FROM public, anon, authenticated;
...
BEGIN
  ...
  WHERE
    ...
    AND (v_construction_statuses IS NULL OR cardinality(v_construction_statuses) = 0 OR b.status::text = ANY(v_construction_statuses))
    AND (v_exclude_construction_statuses IS NULL OR cardinality(v_exclude_construction_statuses) = 0 OR b.status IS NULL OR NOT (b.status::text = ANY(v_exclude_construction_statuses)))
    -- The viewport is unconditional: a text query no longer switches it off.
    -- Same predicate shape as get_map_clusters_v3, including the antimeridian
    -- branch, so both surfaces resolve identical membership for one box.
    AND (((v_safe_max_lng - v_safe_min_lng) > 179 AND b.location::geometry && st_makeenvelope(v_safe_min_lng, v_safe_min_lat, v_safe_max_lng, v_safe_max_lat, 4326)) OR ((v_safe_max_lng - v_safe_min_lng) <= 179 AND b.location && st_makeenvelope(v_safe_min_lng, v_safe_min_lat, v_safe_max_lng, v_safe_max_lat, 4326)::geography))
    -- Parity with get_map_clusters_v3: a building the user hid never draws a
    -- pin, so it must not sit in the list either.
    AND (ub.status::text IS DISTINCT FROM 'ignored')
    AND (v_cities_filter IS NULL OR cardinality(v_cities_filter) = 0 OR b.city = ANY(v_cities_filter))
    AND (v_country_filter IS NULL OR b.country = v_country_filter)
    AND (v_architect_ids IS NULL OR cardinality(v_architect_ids) = 0 OR EXISTS (SELECT 1 FROM building_credits bc WHERE bc.building_id = b.id AND bc.status IS DISTINCT FROM 'hidden'::public.credit_status_enum AND (bc.person_id = ANY(v_architect_ids) OR bc.company_id = ANY(v_architect_ids))))
    ...
    -- 'none' no longer accepts 'ignored': the blanket exclusion above already
    -- removed those rows, so keeping the disjunct would only look meaningful.
    AND (v_status_filter IS NULL OR cardinality(v_status_filter) = 0 OR (('visited' = ANY(v_status_filter) AND ub.status = 'visited') OR ('saved' = ANY(v_status_filter) AND ub.status = 'pending') OR ('none' = ANY(v_status_filter) AND ub.status IS NULL)))
