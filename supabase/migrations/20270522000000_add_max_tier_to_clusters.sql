-- Migration to add max_tier to map clusters for Smart Clusters styling
-- Migration ID: 20270522000000_add_max_tier_to_clusters.sql

-- Drop the function first to ensure clean update of return type
...
DECLARE
  ...
  v_min_rating int; -- Now used for Community Rating (Tier)
  v_personal_min_rating int; -- New: Personal Rating
  v_hide_saved boolean; -- New: Hide Saved
  v_hide_visited boolean; -- New: Hide Visited
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      ELSE
        -- For single pins, calculate it same way for consistency, or NULL if handled by client
        -- But pinStyling.ts uses max_tier for CLUSTERS only.
        -- We can just return NULL or the item's tier. Let's return the item's tier.
        GREATEST(
            CASE
              WHEN max(fb.mapped_rating) >= 3 THEN 3
              WHEN max(fb.mapped_rating) = 2 THEN 2
              ELSE 1
            END,
            CASE
              WHEN max(fb.tier_rank)::text = 'Top 1%' THEN 3
              WHEN max(fb.tier_rank)::text IN ('Top 5%', 'Top 10%') THEN 2
              ELSE 1
            END
        )::int
    END as max_tier
