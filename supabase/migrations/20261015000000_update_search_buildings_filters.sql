-- Update search_buildings RPC to support new taxonomy filters and remove obsolete styles
-- Migration ID: 20261015000000_update_search_buildings_filters.sql
...
DECLARE
  ...
  -- v_styles_filter text[]; -- REMOVED
  v_architect_ids uuid[]; -- UUIDs
  v_category_id uuid; -- Single UUID (Exact Match)
  v_typology_ids uuid[]; -- UUIDs (Any of)
  v_attribute_ids uuid[]; -- UUIDs (Any of)
BEGIN
  -- 1. Parse Inputs
  ...
  IF filters IS NOT NULL THEN
    ...
    -- Functional Category (UUID - Exact Match)
  ...
  -- REMOVED building_styles_agg CTE
  ...
  SELECT
    ...
    b.city,
    b.country,
    '[]'::jsonb as styles, -- Return empty array for schema compatibility
  ...
  -- REMOVED LEFT JOIN building_styles_agg
  WHERE
    ...
    AND
    -- REMOVED Styles Filter
    -- 3. Architects Filter (UUID Match - OR logic within)
    (
    ...
    AND
    -- 4. Functional Category Filter (Exact Match)
    (
        v_category_id IS NULL
        OR b.functional_category_id = v_category_id
    )
    AND
    -- 5. Typologies Filter (OR logic within)
    (
    ...
    AND
    -- 6. Attributes Filter (OR logic within)
    (
    ...
    AND
    -- 9. Text Query (Title, Address, Architect Names)
    -- REMOVED styles from text search
    (
