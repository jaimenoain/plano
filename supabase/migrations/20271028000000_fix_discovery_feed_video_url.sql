-- =============================================================================
-- Migration: fix_discovery_feed_video_url
-- Root cause: Migration 20270872000000 dropped user_buildings.video_url when
--   it migrated note content to building_posts. The get_discovery_feed function
--   still referenced ub.video_url in its HAVING clause, causing a
--   "column ub.video_url does not exist" runtime error on every Explore page load.
-- Fix: replace BOOL_OR(ub.video_url IS NOT NULL) with an equivalent EXISTS
--   subquery against building_posts (where the column now lives).
-- =============================================================================
