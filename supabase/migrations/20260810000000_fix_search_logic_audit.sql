...
BEGIN
  ...
  IF filters IS NOT NULL THEN
    ...
    IF filters ? 'architects' AND jsonb_typeof(filters->'architects') = 'array' THEN
        -- Try to parse as UUIDs. If invalid text is passed, this might throw,
        -- but we assume client sends valid UUIDs if strictly required.
        BEGIN
            SELECT ARRAY(SELECT jsonb_array_elements_text(filters->'architects')::uuid) INTO v_architect_ids;
        EXCEPTION WHEN OTHERS THEN
            -- Fallback or ignore if not UUIDs (e.g. if legacy names were sent)
  ...
  WHERE
    ...
    -- Note: 'description' column does not exist on buildings table, so it is excluded.
    (
