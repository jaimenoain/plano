-- Migration to hide hidden buildings from the map
-- Migration ID: 20270517000000_hide_hidden_in_map.sql
...
BEGIN
  ...
  ELSIF zoom_level >= 11 THEN
    v_grid_size := v_grid_size / 3.0; -- Intermediate step to keep pins separated longer
  END IF;
  ...
  IF filter_criteria IS NOT NULL THEN
    ...
    -- Hide Hidden (New)
    IF filter_criteria ? 'hide_hidden' AND filter_criteria->>'hide_hidden' IS NOT NULL THEN
        v_hide_hidden := (filter_criteria->>'hide_hidden')::boolean;
    ELSE
        v_hide_hidden := false;
    END IF;
  ELSE
  ...
  WITH filtered_buildings AS (
    ...
    WHERE
      ...
      -- Exclude Demolished and Unbuilt buildings
      ...
      AND (v_hide_hidden IS FALSE OR ub.status IS DISTINCT FROM 'ignored')
  )
  SELECT
