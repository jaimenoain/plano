-- Create robust get_map_clusters_v2 function
-- Migration ID: 20270501000000_get_map_clusters_v2.sql
...
BEGIN
  ...
  SELECT
    CASE WHEN count(*) > 1 THEN
      md5(format('%s-%s', st_x(st_snaptogrid(location::geometry, v_grid_size)), st_y(st_snaptogrid(location::geometry, v_grid_size))))::text
    ELSE
      max(id)::text
    END as id,
    ...
    count(*)::int as count,
    CASE WHEN count(*) = 1 THEN max(mapped_rating) ELSE NULL END as rating,
    CASE WHEN count(*) = 1 THEN max(mapped_status) ELSE NULL END as status
  FROM filtered_buildings
  GROUP BY st_snaptogrid(location::geometry, v_grid_size);
END;
$$;
