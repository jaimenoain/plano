-- get_buildings_list: make OFFSET pagination deterministic.
--
-- The SERP list paginates with `LIMIT page_size OFFSET (page-1)*page_size`, but
-- the ORDER BY (rating, popularity/NULL, distance-from-center) is NOT unique:
-- buildings that tie on all three keys have an undefined relative order, so
-- OFFSET can repeat a row on one page and skip another on the next. This was
-- masked while the sidebar's infinite scroll never advanced past page 1; now
-- that it paginates, duplicates surfaced as React duplicate-key warnings and
-- silently-skipped buildings.
--
