-- Fix get_user_groups_summary to use session_buildings/buildings instead of legacy films tables
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      ARRAY(
        SELECT b.main_image_url
        FROM group_sessions gs
        JOIN session_buildings sb ON gs.id = sb.session_id
        JOIN buildings b ON sb.building_id = b.id
        WHERE gs.group_id = g.id AND b.main_image_url IS NOT NULL
