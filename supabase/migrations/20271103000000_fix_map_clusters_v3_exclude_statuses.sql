-- Fix: restore exclude_construction_statuses support in get_map_clusters_v3.
--
-- Root cause: the photography-gap-layer migration (20271019) and the
-- century-filter migration (20271031) both dropped and recreated this function
-- but omitted the `exclude_construction_statuses` branch that was introduced
-- in 20270906. The frontend's resolveConstructionStatuses() always sends
-- exclude_construction_statuses (not construction_statuses) for the default
-- browse path, so the status filter was silently ignored — every building,
-- including Unbuilt and Lost/Demolished, appeared on the photography gap map.
--
-- Fix: add back the variable declaration, the filter_criteria parser, and the
-- WHERE-clause guard. No signature change — CREATE OR REPLACE is sufficient.
--
-- Fixes feedback id 58ffb924-1e2e-4b89-80cd-c235d334aa00 (photography map
-- showing unbuilt / lost buildings).
...
'Browse-only map cluster RPC (Phase 3 + Photography Gaps + Century filter). Supports both inclusion (construction_statuses) and exclusion (exclude_construction_statuses) status filters; exclusion lets NULL-status rows pass through so legacy buildings remain visible.';
