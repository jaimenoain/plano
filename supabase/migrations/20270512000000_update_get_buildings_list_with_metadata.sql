-- Update get_buildings_list to return more metadata for the list view
-- Migration ID: 20270512000000_update_get_buildings_list_with_metadata.sql
...
RETURNS TABLE (
  ...
  rating int,
  status text, -- User status (visited, saved, none)
  city text,
  country text,
  year_completed integer,
  address text,
  physical_status text -- Building status (Built, Unbuilt, etc.)
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    b.year_completed,
    b.address,
    b.status::text as physical_status
  FROM buildings b
  ...
  WHERE
    ...
    -- Conditional bounds check:
    -- If query is present (and not empty), ignore bounds.
    -- Otherwise, apply bounds.
    AND (
