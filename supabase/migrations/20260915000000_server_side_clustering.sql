-- Migration: Server-side clustering
-- Date: 2026-09-15
-- Description: Adds PostGIS, spatial index on buildings, and get_map_clusters RPC.

-- 1. Setup PostGIS
CREATE EXTENSION IF NOT EXISTS postgis;

-- 2. Spatial Indexing
-- The 'buildings' table uses a 'location' GEOGRAPHY(POINT) column (confirmed in schema).
-- We create a GIST index on this dedicated column.
-- Note: 'lat' and 'lng' columns do not exist as standalone columns; they are derived from 'location'.
CREATE INDEX IF NOT EXISTS buildings_location_idx ON buildings USING GIST (location);

-- 3. Core RPC Function: get_map_clusters
...
BEGIN
  ...
  IF zoom < v_cluster_horizon THEN
    ...
    SELECT
      ...
      md5(format('%s-%s', st_x(st_snaptogrid(b.location::geometry, v_grid_size)), st_y(st_snaptogrid(b.location::geometry, v_grid_size))))::text as id,
...
GRANT EXECUTE ON FUNCTION get_map_clusters(double precision, double precision, double precision, double precision, int, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION get_map_clusters(double precision, double precision, double precision, double precision, int, jsonb) TO anon;
GRANT EXECUTE ON FUNCTION get_map_clusters(double precision, double precision, double precision, double precision, int, jsonb) TO service_role;
