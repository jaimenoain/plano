-- Migration to fix image URL logic after column removal

-- 1. Create computed column function 'main_image_url' for buildings table
-- This allows client-side queries like select('*, main_image_url') to work again.
...
BEGIN
  ...
  SELECT
    ...
    jsonb_build_object(
      'id', b.id,
      'name', b.name,
      'main_image_url', main_image_url(b), -- Use computed column function
      'address', b.address,
...
-- 4. Update find_nearby_buildings RPC (Add main_image_url back)
...
RETURNS TABLE (
  ...
  location_precision location_precision,
  main_image_url text -- Added back
)
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  SELECT
    ...
    b.location_precision,
    main_image_url(b) as main_image_url -- Use computed column
  FROM
    buildings b
  WHERE
    b.is_deleted = false
    AND (
      -- 1. Tight radius check
