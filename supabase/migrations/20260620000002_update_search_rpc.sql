-- Migration: Update Search RPC for Normalized Styles

-- Drop old functions (must drop to change return type)
DROP FUNCTION IF EXISTS search_buildings(text, jsonb, int, jsonb, text);
DROP FUNCTION IF EXISTS get_discovery_filters();

-- 1. Updated get_discovery_filters (fetching from normalized tables)
...
BEGIN
  ...
  -- Get distinct styles from the new table
  SELECT jsonb_agg(
    jsonb_build_object(
        'id', id,
        'name', name,
...
-- 2. Updated search_buildings (fetching styles from relation)
CREATE OR REPLACE FUNCTION search_buildings(
  ...
  filters jsonb DEFAULT NULL, -- {cities: text[], country: text, styles: text[]} (styles here are NAMES)
...
BEGIN
  ...
  -- Aggregate Architects (Existing logic)
  ...
  building_styles_agg AS (
    SELECT
        ...
        array_agg(s.name) as styles_names -- for search filtering optimization if needed
  ...
  WHERE
    ...
    AND
    -- Styles Filter (Overlaps via aggregated array)
    (
