-- Roadmap Task 11.1: Remove legacy `architects` / `building_architects`; RPCs use `building_credits` + `people` / `companies`.
-- Apply in Supabase SQL Editor after prior migrations. Drops `architect_affiliations`; keeps `architect_claims` (FK removed).

-- ---------------------------------------------------------------------------
-- RLS: allow claim owner to release profile (set claimed_by_user_id NULL)
-- ---------------------------------------------------------------------------
...
-- Include buildings.community_preview_url in feed RPC building_data so activity cards
-- can show a community thumbnail when hero/main is absent but community photos exist.
...
END;
$$ LANGUAGE plpgsql SECURITY INVOKER;
-- Roadmap Task 10.2: filter map/list RPCs by building_credits (company + role), excluding hidden credits.

CREATE OR REPLACE FUNCTION public.building_matches_credit_filters(
  p_building_id uuid,
  p_company_id uuid,
  p_roles text[]
)
...
AS $f$
  SELECT
    ...
    (p_roles IS NULL OR cardinality(p_roles) = 0 OR EXISTS (
      SELECT 1 FROM public.building_credits bc
      WHERE bc.building_id = p_building_id
        AND bc.role::text = ANY(p_roles)
        AND bc.status IS DISTINCT FROM 'hidden'::public.credit_status_enum
    ));
$f$;

GRANT EXECUTE ON FUNCTION public.building_matches_credit_filters(uuid, uuid, text[]) TO anon;
GRANT EXECUTE ON FUNCTION public.building_matches_credit_filters(uuid, uuid, text[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.building_matches_credit_filters(uuid, uuid, text[]) TO service_role;
...
BEGIN
  ...
  IF filters IS NOT NULL THEN
    ...
    IF filters ? 'credit_company_id' AND filters->>'credit_company_id' IS NOT NULL THEN
      BEGIN
        v_credit_company_id := (filters->>'credit_company_id')::uuid;
      EXCEPTION WHEN OTHERS THEN
        v_credit_company_id := NULL;
      END;
    END IF;
  ...
  AND (b.status::text IS DISTINCT FROM 'Demolished' AND b.status::text IS DISTINCT FROM 'Lost' AND b.status::text IS DISTINCT FROM 'Unbuilt')
    ...
    AND (
        ...
        OR EXISTS (
            ...
            WHERE bc.building_id = b.id
            AND bc.status IS DISTINCT FROM 'hidden'::public.credit_status_enum
            AND (bc.person_id = ANY(v_architect_ids) OR bc.company_id = ANY(v_architect_ids))
        )
    )
...
BEGIN
  ...
  IF filter_criteria IS NOT NULL THEN
    ...
    IF filter_criteria ? 'credit_company_id' AND filter_criteria->>'credit_company_id' IS NOT NULL THEN
      BEGIN
        v_credit_company_id := (filter_criteria->>'credit_company_id')::uuid;
      EXCEPTION WHEN OTHERS THEN
        v_credit_company_id := NULL;
      END;
    END IF;
...
-- OUT/return row shape changed (architects → credit_names); REPLACE is not allowed.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      (
        SELECT array_agg(DISTINCT COALESCE(p.name, c.name)) FILTER (WHERE COALESCE(p.name, c.name) IS NOT NULL)
        FROM building_credits bc
        LEFT JOIN people p ON bc.person_id = p.id
        LEFT JOIN companies c ON bc.company_id = c.id
        WHERE bc.building_id = b.id
...
-- OUT/return row shape changed (architects jsonb → credited_entities jsonb); REPLACE is not allowed.
DROP FUNCTION IF EXISTS search_buildings(text, double precision, double precision, integer, jsonb, text, text[], text[], text[], integer);
...
BEGIN
  ...
  IF filters IS NOT NULL THEN
    ...
    IF filters ? 'architect_ids' AND jsonb_typeof(filters->'architect_ids') = 'array' THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            v_architect_ids := NULL;
        END;
    ELSIF filters ? 'people' AND jsonb_typeof(filters->'people') = 'array' THEN
        BEGIN
            SELECT ARRAY(SELECT jsonb_array_elements_text(filters->'people')::uuid) INTO v_architect_ids;
        EXCEPTION WHEN OTHERS THEN
            v_architect_ids := NULL;
        END;
    END IF;
  ...
  building_credits_agg AS (
    SELECT
      ...
      ) FILTER (WHERE COALESCE(p.name, c.name) IS NOT NULL) AS credited_json,
      array_agg(DISTINCT COALESCE(p.name, c.name)) FILTER (WHERE COALESCE(p.name, c.name) IS NOT NULL) AS credit_names
    FROM building_credits bc
    LEFT JOIN people p ON bc.person_id = p.id
    LEFT JOIN companies c ON bc.company_id = c.id
  ...
  SELECT
    ...
    b.country,
    '[]'::jsonb as styles,
    COALESCE(ba_agg.credited_json, '[]'::jsonb) as credited_entities,
    b.year_completed,
    CASE
  ...
  LEFT JOIN building_credits_agg ba_agg ON b.id = ba_agg.building_id
  WHERE
...
ALTER TABLE public.architect_claims
  DROP CONSTRAINT IF EXISTS architect_claims_architect_id_fkey;

ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_verified_architect_id_fkey;

ALTER TABLE public.notifications
  DROP CONSTRAINT IF EXISTS notifications_architect_id_fkey;

DROP TABLE IF EXISTS public.architect_affiliations;

DROP TABLE IF EXISTS public.building_architects;

DROP TABLE IF EXISTS public.architects;

DROP TYPE IF EXISTS public.architect_type;
