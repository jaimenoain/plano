...
BEGIN
  ...
  WITH all_user_activities AS (
    SELECT user_id, MAX(created_at) as last_activity
    FROM (
      SELECT user_id, created_at FROM log
      UNION ALL SELECT user_id, created_at FROM comments
      UNION ALL SELECT user_id, created_at FROM session_comments
      UNION ALL SELECT user_id, created_at FROM likes
      UNION ALL SELECT user_id, created_at FROM session_likes
      UNION ALL SELECT user_id, created_at FROM comment_likes
      UNION ALL SELECT user_id, created_at FROM poll_votes
      UNION ALL SELECT user_id, created_at FROM login_logs
    ) all_actions
    GROUP BY user_id
  ),
  user_stats AS (
    SELECT
      p.id,
      COALESCE(act.last_activity, p.created_at) as last_active_at,
      CASE
        WHEN act.last_activity > (NOW() - INTERVAL '30 days') THEN 'active_30d'
        WHEN act.last_activity > (NOW() - INTERVAL '90 days') THEN 'active_90d'
        ELSE 'inactive'
      END as status
    FROM profiles p
    LEFT JOIN all_user_activities act ON p.id = act.user_id
    WHERE (p.role IS DISTINCT FROM 'test_user' AND p.role IS DISTINCT FROM 'admin')
  ),
