-- Fix: gap photo count should only include public photos.
--
-- Root cause: the photos_count subquery in get_map_clusters_v3 was:
--
--   SELECT ub2.building_id, COUNT(ri.id) AS photos_count
--   FROM user_buildings ub2
--   JOIN review_images ri ON ri.review_id = ub2.id
--   GROUP BY ub2.building_id
--
-- This counts ALL review_images for a building, including those attached to
-- building_posts with visibility = 'contacts' or visibility = 'private'.
-- Migration 20270872000000 re-pointed review_images.review_id to
-- building_posts.id (same UUIDs preserved), and building_posts carries a
-- visibility column. The correct count is: public posts only.
--
-- Fix: join through building_posts and add WHERE COALESCE(visibility,'public') = 'public'.
-- "No photos" / "Fewer than 3" / "3+ photos" filter chips on the photography
-- gap map now reflect public photos only, as the reporter expects.
--
-- Fixes feedback id a48b4a74-7037-4074-b14e-4ac36ee8d9a9.
...
'Browse-only map cluster RPC (Phase 3 + Photography Gaps + Century filter). photos_count counts only public building_posts (visibility = public or NULL). Supports both inclusion (construction_statuses) and exclusion (exclude_construction_statuses) status filters.';
