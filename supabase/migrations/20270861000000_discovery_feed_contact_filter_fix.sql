-- Fix get_discovery_feed: contact-user filter must still return rows when the viewer
-- already has a user_buildings row for the same building (e.g. same saves as the
-- picked user, or filtering by own activity). The discovery NOT EXISTS guard only
-- applies when no contact filter is active.
