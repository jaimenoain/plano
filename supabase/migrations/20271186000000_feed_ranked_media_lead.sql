-- =============================================================================
-- Migration: feed_ranked_media_lead
-- Purpose:   Guarantee the home feed opens on a post that carries media.
--
-- Until now the "first card must lead with media" rule was enforced purely on
-- the client (`promoteFirstMediaEntry`), which can only reorder the page it has
-- already fetched. When a whole page of `get_feed_ranked` happened to contain
-- no media-bearing post — which is the live state of §01 for real accounts —
-- there was nothing to promote and the feed opened on a text-only note.
--
-- Both rankers now pick the highest-scoring media-bearing post out of the FULL
-- candidate set (not just the requested page) and sort it to the very front.
-- The test is the post's OWN media:
--   * `image_cnt` — rows in `review_images` for that post, and
--   * `has_video` — that post's `building_posts.video_url`.
-- The building's `main_image_url` / `community_preview_url` are NOT considered;
-- a note about a well-photographed building is still a text-only note.
--
-- The promotion is part of the total ordering (not a page-1 special case), so
-- pagination stays gap-free and duplicate-free: the promoted row simply leaves
-- its score position and takes the lead.
--
...
BEGIN
  ...
  -- Best media-bearing post across ALL candidates. Media = the post's own
  -- photos (review_images) or its own video — never the building's imagery.
  lead_media AS (
    SELECT ra.id
    FROM ranked_all ra
    WHERE ra.image_cnt > 0 OR ra.has_video
    ORDER BY ra.score DESC, ra.id DESC
    LIMIT 1
  ),
  ranked AS (
    SELECT
      ra.*,
      COALESCE(ra.id = (SELECT lm.id FROM lead_media lm), FALSE) AS is_lead
    FROM ranked_all ra
    ORDER BY
      COALESCE(ra.id = (SELECT lm.id FROM lead_media lm), FALSE) DESC,
      ra.score DESC,
      ra.id DESC
  ...
  ORDER BY r.is_lead DESC, r.score DESC, r.id DESC;
END;
$$;
