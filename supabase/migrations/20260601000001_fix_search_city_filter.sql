...
-- Replaced to fix city filter logic (expecting array 'cities' instead of string 'city')
...
BEGIN
  ...
  WHERE
    ...
    (v_cities_filter IS NULL OR b.city = ANY(v_cities_filter))
    AND
    -- Country Filter
    (v_country_filter IS NULL OR b.country = v_country_filter)
    AND
    -- Styles Filter (Overlaps)
    (v_styles_filter IS NULL OR b.styles && v_styles_filter)
    AND
