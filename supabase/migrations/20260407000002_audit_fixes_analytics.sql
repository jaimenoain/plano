-- Fix Analytics Audit Issues: Radar Normalization & Sparkline Stability
-- 1. Updates `calculate_scope_stats` to normalize Radar Chart (min 2 films per genre).
-- 2. Updates `update_group_stats` to fix Sparkline sorting stability.

-- A. Update calculate_scope_stats (Radar Chart Fix)
...
BEGIN
    ...
    expanded_genres AS (
        -- Audit Fix: Added film_id to allow distinct counting
        SELECT l.user_id, l.rating, g.id as genre_id, g.name as genre_name, l.film_id
        FROM scope_logs l JOIN target_genres g ON g.id = ANY(l.genre_ids)
    ),
    group_radar AS (
        -- Audit Fix: Enforce minimum of 2 films per genre to prevent single-film outliers from spiking multiple axes
        SELECT t.name as subject,
        CASE
            WHEN COUNT(DISTINCT e.film_id) >= 2 THEN COALESCE(AVG(e.rating), 0)
            ELSE 0
        END as value,
        10 as "fullMark"
...
-- B. Update update_group_stats (Sparkline Stability Fix)
...
BEGIN
    ...
    FROM (
        ...
        GROUP BY film_id
        HAVING COUNT(id) >= 2 -- EXCLUDE FILMS WITH LESS THAN 2 RATINGS
        -- Audit Fix: Added deterministic secondary sorting to prevent sparkline jitter
        ORDER BY avg_rating DESC, review_count DESC, title ASC
    ) t;
