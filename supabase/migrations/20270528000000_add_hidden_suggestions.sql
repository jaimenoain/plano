CREATE TABLE IF NOT EXISTS suggested_profile_hides (
...
ALTER TABLE suggested_profile_hides ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can insert their own hides" ON suggested_profile_hides;
CREATE POLICY "Users can insert their own hides"
    ON suggested_profile_hides FOR INSERT
    WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view their own hides" ON suggested_profile_hides;
CREATE POLICY "Users can view their own hides"
    ON suggested_profile_hides FOR SELECT
    USING (auth.uid() = user_id);

-- Update the function to exclude hidden users
...
BEGIN
  ...
  candidate_scores AS (
    ...
    AND NOT EXISTS (
      SELECT 1 FROM my_hides mh WHERE mh.suggested_user_id = p.id
    )
  )
  SELECT
    cs.id,
