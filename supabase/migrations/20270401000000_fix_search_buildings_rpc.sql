-- Fix search_buildings RPC signature and filter logic
-- Migration ID: 20270401000000_fix_search_buildings_rpc.sql
...
BEGIN
  ...
  WHERE
    ...
    (
      ...
      array_to_string(ba_agg.architects_names, ' ') ILIKE '%' || v_query || '%'
      OR
...
GRANT EXECUTE ON FUNCTION search_buildings(text, double precision, double precision, int, jsonb, text, int) TO authenticated;
GRANT EXECUTE ON FUNCTION search_buildings(text, double precision, double precision, int, jsonb, text, int) TO anon;
GRANT EXECUTE ON FUNCTION search_buildings(text, double precision, double precision, int, jsonb, text, int) TO service_role;
