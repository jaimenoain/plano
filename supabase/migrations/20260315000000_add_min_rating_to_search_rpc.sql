...
-- Re-create the function with the correct signature (added p_min_rating, p_friends_min_rating)
CREATE OR REPLACE FUNCTION search_films_tiered(
    ...
    p_friends_min_rating INTEGER DEFAULT NULL
)
RETURNS TABLE (
    id TEXT,
    tmdb_id INTEGER,
...
BEGIN
    ...
    WITH tier1_candidates AS (
        ...
        AND (p_seen_by_user_id IS NULL OR EXISTS (
            SELECT 1 FROM log l_s WHERE l_s.film_id = f.id AND l_s.user_id = p_seen_by_user_id AND l_s.status = 'watched' AND (p_min_rating IS NULL OR l_s.rating >= p_min_rating)
        ))
        ...
        AND (p_rated_by_user_ids IS NULL OR EXISTS (
            SELECT 1 FROM log l_r WHERE l_r.film_id = f.id AND l_r.user_id = ANY(p_rated_by_user_ids) AND l_r.rating IS NOT NULL AND (p_friends_min_rating IS NULL OR l_r.rating >= p_friends_min_rating)
        ))
        ORDER BY f.id
    ),
    tier2_candidates AS (
