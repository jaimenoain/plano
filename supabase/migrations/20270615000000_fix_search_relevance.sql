-- Migration to improve search relevance in get_buildings_list RPC
-- Migration ID: 20270615000000_fix_search_relevance.sql

-- Update get_buildings_list to prioritize search query matches over distance
...
BEGIN
  ...
  ORDER BY
    ...
    -- Relevance Sorting (Exact > Starts > Contains > Fuzzy)
    CASE
        -- Exact Match (Name or Alt Name)
        WHEN v_query IS NOT NULL AND (b.name ILIKE v_query OR b.alt_name ILIKE v_query) THEN 100
        -- Starts With (Name or Alt Name)
        WHEN v_query IS NOT NULL AND (b.name ILIKE v_query || '%' OR b.alt_name ILIKE v_query || '%') THEN 80
        -- Contains (Name or Alt Name)
        WHEN v_query IS NOT NULL AND (b.name ILIKE '%' || v_query || '%' OR b.alt_name ILIKE '%' || v_query || '%') THEN 60
        -- Fuzzy (Name only for now to keep it simple/fast)
        WHEN v_query IS NOT NULL AND LENGTH(v_query) > 2 THEN similarity(b.name, v_query) * 50
        ELSE 0
    END DESC,
