-- Migration to exclude test users from admin stats and mark tester@cineforum.eu

-- 1. Mark the test user
UPDATE profiles
SET role = 'test_user'
WHERE id IN (SELECT id FROM auth.users WHERE email = 'tester@cineforum.eu');
...
BEGIN
  ...
  -- Session Reliability
  -- Simplified metric, but filtering interactions by non-test users
