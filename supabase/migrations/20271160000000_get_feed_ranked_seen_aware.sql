-- =============================================================================
-- Migration: get_feed_ranked_seen_aware
-- Purpose:   Make the followed-users ranker seen-aware. Adds a third parameter
--            p_exclude_seen (default true). When true, posts the viewer has
--            already seen (recorded in note_views) are dropped from the
--            candidate pool, so a returning user only sees *new* updates from
--            people they follow. Own posts are never in note_views (self-views
--            are filtered at ingestion), so they are unaffected.
--
-- Everything else is identical to the original get_feed_ranked (migration
-- 20270911000000): same scoring formula, same returned columns, ring='direct'.
-- =============================================================================

BEGIN;

-- Drop the original 2-arg signature so the 3-arg version is unambiguous.
...
-- PL/pgSQL variables. (The original get_feed_ranked omitted this and errored
-- at runtime with "column reference user_id is ambiguous".)
...
BEGIN
  ...
  WITH candidates AS (
    ...
    WHERE
      ...
      -- Seen-aware: when enabled, drop posts the viewer has already seen.
      -- Own posts never appear in note_views (self-views filtered at ingestion).
      AND (
        NOT p_exclude_seen
        OR NOT EXISTS (
