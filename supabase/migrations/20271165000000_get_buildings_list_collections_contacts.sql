-- get_buildings_list: wire the Folders/Collections and Curators & friends
-- filters (previously dead on the SERP list) via the shared helpers
-- get_buildings_in_collections + building_matches_contact_filters. Both are
-- guarded so an unset filter is a no-op. Builds on the Slice-1 parity version.
