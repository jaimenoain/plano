-- search_buildings_v2 (Find mode, feeds BOTH the SERP list and map pins):
-- bring construction-status + century semantics in line with the Browse RPCs.
--   * exclude_construction_statuses: new support so the Building-status filter /
--     "Show lost" toggle behave the same in Find mode as in Browse. Driven by the
--     client (SearchPage.findModeFilters via resolveConstructionStatuses); no
--     server-side default is baked in, so other search_buildings_v2 semantics are
--     unchanged when the key is absent.
--   * Century B.C. (id 0): adopt the map's B.C.-aware clause (0 -> century < 1),
--     replacing the exact `century = ANY(...)` match that only hit rows with
--     century = 0.
-- Signature unchanged — body only.
