...
- **New RPC? `revoke ... from public, anon, authenticated` — naming the roles is mandatory.** `from public` alone does *not* lock a function down: Supabase's `ALTER DEFAULT PRIVILEGES` grants `EXECUTE` to `anon` and `authenticated` **directly** at creation, and revoking the `PUBLIC` pseudo-role leaves those grants in place. This shipped a live hole (anonymous `rpc/run_weekly_digest` returned 200, #1671). Grant back only the roles that need it, then verify with `has_function_privilege('anon', 'public.fn(args)', 'EXECUTE')`. Start from `supabase/migrations/_TEMPLATE_rpc.sql.txt`; the RLS-policy and `SECURITY INVOKER` caller traps to check **before revoking an existing** function are in [`docs/migrations.md`](docs/migrations.md#writing-an-rpc-database-function).
