...
test('Verify Building Details Summary Card vs Edit Form', async ({ page }) => {
  ...
  await page.route('**/rest/v1/user_buildings*', async route => {
      ...
      const url = route.request().url();
      if (url.includes('user_id=eq.user-uuid') && url.includes('building_id=eq.b1')) {
          await route.fulfill({
              status: 200,
              contentType: 'application/json',
              body: JSON.stringify({
                  ...
                  tags: ['Cool', 'Modern'],
                  images: [
                      { id: 'img1', storage_path: 'user-uuid/b1/photo1.jpg' }
                  ]
              })
          });
      } else {
  ...
  // The textarea has placeholder "Write a private note..."
  await expect(page.getByPlaceholder('Write a private note...')).toBeHidden();
