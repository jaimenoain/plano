...
- **Accent:** `--brand-accent` (#BEFF00) — the four sanctioned uses only (see below).
...
Anywhere else — section accents, surface fills, verified badges, decorative splashes — it's a bug. The logo is always `currentColor`, never recoloured to lime.
...
- Awards, not ratings. Dots are Michelin-style honours on an ascending adjective ladder (**Interesting 0 · Impressive 1 · Essential 2 · Masterpiece 3**), never stars, never "out of 3". **0 dots (Interesting) is the default** — and even it is a compliment, since saving a building already means it earned a place; a dot is a further distinction, not a low score. Show only filled **black** dots (never lime, never outlined); zero dots shows nothing. Never pad with empty rings in display *or* input — one dot beside two empty rings reads as a bad score, the opposite of the intent. Input = discrete labelled tiers, never left-to-right fillable slots.
...
- New colours, or lime outside its four uses.
...
- Star ratings · empty/outlined award dots · dots shown "out of 3" · a rating input built as fillable slots · rounded/pill buttons · ALL-CAPS sentences.
...
- `colors_and_type.css` — all tokens, semantic element styles, plus `.photo-placeholder` and `.accent-tag` utilities. **Drop it into any HTML and it looks Plano.**
- `assets/plano-logo.svg` — wordmark (uses `currentColor`).
- `preview/*.html` — canonical cards for design-review.
- `ui_kits/web/Plano Web.html` — working recreation of landing, feed, and building-detail surfaces. Use as a reference when building new surfaces.
