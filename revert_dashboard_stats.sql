-- Fix Production Crash: Revert API Interface for Old Frontend Compatibility
-- This temporarily modifies get_admin_dashboard_stats to return 'activity_trends' as an Array (v_actions)
-- instead of an Object, matching the expectation of the currently live (old) frontend code.
-- It retains the new logic (excluding test users, etc.) but downgrades the JSON structure.
...
BEGIN
  ...
  -- Most Recently Online (Still kept here for backward compatibility or direct use if needed)
  top_online AS (
  ...
  -- Most Sessions (Logged in visits)
  top_sessions AS (
    SELECT l.user_id, p.username, p.avatar_url, COUNT(*) as count
    FROM login_logs l
    JOIN profiles p ON l.user_id = p.id
  ...
  RETURN json_build_object(
    ...
    'activity_trends', COALESCE(v_actions, '[]'::json), -- !!! REVERT FIX: Return array directly
