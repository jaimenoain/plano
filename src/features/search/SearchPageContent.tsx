/**
 * SearchPageContent — the search/map surface itself, extracted from
 * SearchPage.tsx (the route module keeps the loaders + provider shell).
 * Refined with the A24 editorial aesthetic:
 *
 * Desktop sidebar:
        ...
        */}
        <div className="absolute bottom-0 left-0 top-0 z-20 hidden w-search-serp min-h-0 flex-col border-r border-border-default bg-surface-card transition-all duration-300 md:flex">
          ...
          {/* Sidebar search header — search + filters and the All / Discover
              switch form one grouped block so the destination toggle reads as
              part of the query controls, not a detached band. */}
