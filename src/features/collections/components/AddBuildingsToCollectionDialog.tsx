...
import { OtherMarkersTabPanel } from "./OtherMarkersTabPanel";
...
toast.error("Failed to add building");
  ...
  // The return trip from /add-building is a client navigation, so the list query
  // survives it cached under the global 5-min staleTime — the just-created building
  // would be missing from the SERP (and from `selectedBuilding`) until a hard
  // refresh. Drop the whole key prefix once: both the saved-buildings entry and the
  // "no results" entry cached for the term searched just before creating are stale.
  const refreshedForCreatedRef = useRef<string | null>(null);
  useEffect(() => {
    if (!justCreatedBuildingId) return;
    if (refreshedForCreatedRef.current === justCreatedBuildingId) return;
    refreshedForCreatedRef.current = justCreatedBuildingId;
    void queryClient.invalidateQueries({ queryKey: ["add-buildings-dialog"] });
  }, [justCreatedBuildingId, queryClient]);
