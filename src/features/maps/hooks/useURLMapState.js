...
export const MapFiltersObjectSchema = z.object({
    ...
    globalMinRating: z.preprocess((val) => {
        if (typeof val === 'string' && val.trim() !== '') {
            const num = Number(val);
            return isNaN(num) ? undefined : num;
        }
    ...
    personalMinRating: z.preprocess((val) => {
        if (typeof val === 'string' && val.trim() !== '') {
            const num = Number(val);
            return isNaN(num) ? undefined : num;
        }
...
export const useURLMapState = () => {
    ...
    const mapState = useMemo(() => {
        ...
        const getArrayParam = (param) => param ? param.split(",") : undefined;
        const getBoolParam = (param) => param === "true" ? true : undefined;
        const getNumParam = (param) => param ? parseInt(param, 10) : undefined;
        const getIdListParam = (param) => param ? param.split(",").map(id => ({ id, name: id })) : undefined;
        const parseMichelin = (param) => {
    ...
    const setMapURL = useCallback((updates) => {
        setSearchParams((prev) => {
            const newParams = new URLSearchParams(prev);
            if (updates.lat !== undefined)
                newParams.set('lat', updates.lat.toString());
            if (updates.lng !== undefined)
                newParams.set('lng', updates.lng.toString());
