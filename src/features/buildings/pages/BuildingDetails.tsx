...
export function HydrateFallback() {
  return (
    <AppLayout showBack title="Loading...">
      <Skeleton className="w-full h-[clamp(260px,48vh,500px)]" />
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto space-y-6 mt-6">
...
export function ErrorBoundary() {
    ...
    return (
      <AppLayout showBack>
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 py-8 text-center">
          <h1 className="text-2xl font-bold tracking-tight text-text-primary mb-2">Building not found</h1>
          <p className="text-text-secondary max-w-md mb-6 text-sm md:text-base leading-relaxed">
            ...
            {pathHint ? <> <span className="font-mono text-text-primary">({pathHint})</span></> : null}.
  ...
  return (
    <AppLayout showBack>
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 py-8 text-center">
        <h1 className="text-2xl font-bold tracking-tight text-text-primary mb-2">Something went wrong</h1>
        <p className="text-text-secondary max-w-md mb-6 text-sm md:text-base leading-relaxed">
          An unexpected error occurred while loading this building.
        </p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <Button type="button" size="lg" variant="default" className="min-w-[200px]"
            onClick={() => revalidator.revalidate()} disabled={revalidator.state === "loading"}>
            Try again
          </Button>
...
export default function BuildingDetails() {
  ...
  const canEditOfficialData =
    profile?.role === "admin" || isVerifiedArchitect || (isCreator && !hasVerifiedArchitect);
  ...
  // Upload / link / collection editing state (used in WriteReview page)
  ...
  // --- Access synthesis (previously computed inside BuildingHeader) ---
  const accessSynthesis = useMemo(() => {
    if (!building || (!building.access_level && !building.access_logistics && !building.access_cost))
      return null;
  ...
  const googleSearchUrl = useMemo(() => {
    if (!building) return "";
    const query = [building.name, building.city, building.architects?.map((a) => a.name).join(" ")]
      .filter(Boolean)
      .join(" ");
  ...
  const fetchUserSpecificData = async () => {
    ...
    try {
      ...
      // =======================================================================
      // RECONSTRUCTED SECTION — verify against your original source
      // Original file lines: ~71101–71292 (truncated in repomix export)
      // The block below matches the surrounding patterns. If your original
      // differs, paste the original content between these markers.
      // =======================================================================
      if (user) {
        tasks.push(
          (async () => {
            // Architect claim verification for this user
            const { data: claimsData } = await supabase
              ...
              .eq("status", "approved");
            if (claimsData) {
              setVerifiedClaims(claimsData.map((c) => c.architect_id));
            }

            // User's personal entry for this building
            const { data: ubData } = await supabase
              .from("user_buildings")
              .select("status, rating, content, edited_at")
              .eq("user_id", user.id)
              .eq("building_id", resolvedBuildingId)
              .maybeSingle();
            if (ubData) {
              setUserStatus(ubData.status as "visited" | "pending" | "ignored");
              setMyRating(ubData.rating || 0);
              setNote(ubData.content || "");
            }

            // User's images for this building
            const { data: userImgData } = await supabase
              .from("building_images")
              .select("id, storage_path, is_generated, is_official")
              .eq("building_id", resolvedBuildingId)
              .eq("uploaded_by", user.id);
            if (userImgData) setUserImages(userImgData);
            ...
            const { data: colData } = await supabase
              ...
              .eq("building_id", resolvedBuildingId);
            if (colData) {
              const colIds = colData.map((c) => c.collection_id).filter(Boolean) as string[];
              const { data: ownedCols } = await supabase
                .from("collections")
                .select("id")
                .in("id", colIds)
                .eq("created_by", user.id);
              const ownedIds = (ownedCols || []).map((c) => c.id);
              setSelectedCollectionIds(ownedIds);
              setInitialCollectionIds(ownedIds);
            }
          })(),
        );
      }
      ...
      tasks.push(
        (async () => {
          ...
          if (!reviewsError && reviewsData) {
            const mapped = (reviewsData as RpcBuildingReviewRow[]).map((row) => ({
              id: row.id,
              user_id: row.user_id,
              content: row.content ?? null,
              rating: row.rating ?? null,
              status: (row.status ?? "visited") as "visited" | "pending",
              tags: row.tags ?? null,
              created_at: row.created_at,
              user: row.user_data ?? { username: null, avatar_url: null },
              images: row.images ?? [],
            })) as FeedEntry[];
            setEntries(mapped);
          }
      ...
      tasks.push(
        (async () => {
          const { data: allImages } = await supabase
            .from("building_images")
            .select("id, storage_path, is_generated, is_official, likes_count, created_at")
            .eq("building_id", resolvedBuildingId)
            .order("is_official", { ascending: false })
            .order("likes_count", { ascending: false });

          if (allImages) {
            const communityImages: DisplayImage[] = allImages
              .map((img) => {
            ...
            if (user && communityImages.length > 0) {
              const imageIds = communityImages.map((img) => img.id);
  ...
  const fetchTopLinks = async (buildingId: string) => {
    ...
    const { data: linksData, error: linksError } = await supabase.rpc("get_building_top_links", {
  ...
  const handleLinkLike = async (linkId: string) => {
    ...
    setTopLinks((prev) =>
      prev.map((l) =>
        l.link_id === linkId ? { ...l, like_count: l.like_count + (isLiked ? -1 : 1) } : l,
      ),
    );
    try {
      if (isLiked) {
      ...
      for (const file of files) {
        try {
          ...
          setPendingImages((prev) => [
            ...prev,
            { id: crypto.randomUUID(), file: compressedFile, preview: previewUrl, is_generated: false },
          ]);
        } catch (_error) {
  ...
  // =======================================================================
  // RECONSTRUCTED SECTION — verify against your original source
  // Original file lines: ~71500–71699 (not captured in repomix export)
  // Contains: handleSaveNote, handleSaveOfficialData, handleSendInvites,
  //           handleDelete, handleSetHeroImage (start of handleToggleOfficial)
  // =======================================================================
  const handleSaveNote = async () => {
    ...
    try {
      ...
      for (const pending of pendingImages) {
        const path = `buildings/${building.id}/${user.id}/${crypto.randomUUID()}`;
        const { error: uploadError } = await uploadFile(pending.file, path);
        if (!uploadError) {
          await supabase.from("building_images").insert({
            building_id: building.id,
            storage_path: path,
            uploaded_by: user.id,
            is_generated: pending.is_generated,
          });
        }
      }
      // Save links
      ...
      if (error) throw error;
      // Sync collections
      const removed = initialCollectionIds.filter((id) => !selectedCollectionIds.includes(id));
      const added = selectedCollectionIds.filter((id) => !initialCollectionIds.includes(id));
      ...
      for (const colId of added) {
        await supabase
          .from("collection_buildings")
          .insert({ collection_id: colId, building_id: building.id });
      }
      setPendingImages([]);
      setUserLinks([]);
  ...
  const handleSendInvites = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast({
        title: `Invite sent to ${selectedFriends.length} friend${selectedFriends.length > 1 ? "s" : ""}`,
      });
      setSelectedFriends([]);
      setShowVisitWith(false);
    } catch (_error) {
      toast({ variant: "destructive", title: "Failed to send invites" });
    } finally {
  ...
  const handleSetHeroImage = async (imageId: string) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq("id", building.id);
      if (error) throw error;
      setBuilding((prev) => (prev ? { ...prev, hero_image_id: imageId } : null));
      toast({ title: "Hero image updated" });
    } catch (_error) {
      toast({ variant: "destructive", title: "Failed to update hero image" });
    }
  };

  const handleToggleOfficial = async (imageId: string, newStatus: boolean) => {
    if (!selectedImage) return;
    setSelectedImage((prev) => (prev ? { ...prev, is_official: newStatus } : null));
    setDisplayImages((prev) =>
      prev.map((img) => (img.id === imageId ? { ...img, is_official: newStatus } : img)),
    );
    try {
      const { error } = await supabase
        .from("building_images")
        ...
        .eq("id", imageId);
      if (error) throw error;
      toast({ title: newStatus ? "Marked as official" : "Removed from official" });
    } catch (_error) {
  ...
  // --- Editorial gallery renderer ---
  const renderEditorialGrid = useCallback(
    (images: DisplayImage[]) => {
      if (images.length === 0) return null;
      const [first, second, third, ...rest] = images;

      const FeatureImg = ({ img }: { img: DisplayImage }) => (
        <div className="overflow-hidden w-full h-full cursor-pointer" onClick={() => setSelectedImage(img)}>
          {img.type === "video" ? (
      ...
      return (
        <div>
          {/* Feature row: 1 large + 2 stacked small */}
          <div
            className="grid gap-0.5"
            style={{
              gridTemplateColumns: second || third ? "3fr 2fr" : "1fr",
      ...
      return (
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="all" className="mt-0">
            {renderEditorialGrid(displayImages)}
          </TabsContent>
          <TabsContent value="official" className="mt-0">
            {renderEditorialGrid(officialImages)}
          </TabsContent>
        </Tabs>
      );
    }
        ...
        <BuildingHero key={heroImageUrl} src={heroImageUrl} alt={building.name}>
          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
            ...
            <div className="flex items-center gap-3 mt-2 text-sm text-white/60">
              {building.architects?.[0]?.name && (
                <span className="text-brand-primary font-semibold">{building.architects[0].name}</span>
              )}
              <span className="text-white/30">·</span>
              <span className="font-mono">{building.year_completed}</span>
            </div>
          </div>
        </BuildingHero>
  ...
  return (
    <AppLayout title={building.name} showBack>
                      ...
                      <div
                        ...
                        className={`w-2 h-2 rounded-full border ${
                          i <= Math.round(avgRating)
                            ? "bg-brand-primary border-brand-primary-foreground"
              ...
              {isOfficialEditing ? (
                <>
                  ...
                  <Button
                    ...
                    disabled={isSavingOfficial}
                    className="bg-brand-primary text-brand-primary-foreground hover:opacity-90 h-7"
                  >
                    {isSavingOfficial && <Loader2 className="w-3 h-3 mr-1.5 animate-spin" />}
                    Save
                  </Button>
                </>
              ) : (
                <Button
                  ...
                  className="text-white/45 hover:text-white hover:bg-white/10 h-7 w-7"
      ...
      {/* ── Official data editing panel (shown below hero when editing) ── */}
      {isOfficialEditing && (
        <div className="border-b border-border-default bg-surface-muted/50 px-4 sm:px-6 lg:px-8 py-5">
          <div className="max-w-7xl mx-auto">
            <p className="text-[10px] font-bold tracking-[0.12em] uppercase text-text-disabled mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDraftOfficialData((p) => ({ ...p, year_completed: parseInt(e.target.value) }))
                  }
                  className="h-8 text-sm"
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => setDraftOfficialData((p) => ({ ...p, city: e.target.value }))}
                  className="h-8 text-sm"
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => setDraftOfficialData((p) => ({ ...p, country: e.target.value }))}
                  className="h-8 text-sm"
                />
              </div>
            </div>
      ...
      {/* ── Building status alert (Lost / Unbuilt / Under Construction) ── */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10 lg:gap-14 items-start">
            ...
            <div className="space-y-10 min-w-0">
              ...
              <section>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="text-xs h-7 text-text-secondary hover:text-text-primary"
                ...
                <div className="mt-3 text-center">
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 text-[11px] text-text-disabled hover:text-brand-primary hover:underline transition-colors"
                  >
              ...
              <section className="pt-10 border-t border-border-default">
                <h2 className="text-[10px] font-bold tracking-[0.12em] uppercase text-text-disabled mb-4">
                  Location
                </h2>
                ...
                {building.location_precision === "approximate" && (
                  <Alert className="border-amber-500/50 bg-amber-500/10 text-amber-500 mb-4">
                    ...
                    <AlertDescription className="ml-2">
                      Exact location not verified. This marker indicates the general
                      village/locality.
                    </AlertDescription>
                  </Alert>
                )}
                ...
                <div className="flex items-center justify-between gap-4 mt-4 flex-wrap">
                  <div className="flex items-center gap-2 text-text-secondary text-sm font-medium group">
                    ...
                    {user && !isOfficialEditing && (
                      <Link
                        to={
                          getBuildingUrl(building.id, building.slug, building.short_id) + "/edit"
                        }
                  ...
                  {coordinates && (
                    <>
                      ...
                      >
                        {building.status === "Lost"
                          ? building.location_precision === "approximate"
                            ? "Navigate to Site (Approx.)"
                            : "Navigate to Site"
                          : building.location_precision === "approximate"
                            ? "Get Directions (Approx.)"
                            : "Get Directions"}
                      </Button>
            ...
            <div className="flex flex-col gap-4 lg:sticky lg:top-4 lg:self-start">
              ...
              <div className="border border-border-default bg-surface-card">
                <div className="px-4 py-3 border-b border-border-default">
                  <span className="text-[10px] font-bold tracking-[0.12em] uppercase text-text-disabled">
                    Your Activity
                  </span>
                </div>
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-1.5">
                    <Button
                      ...
                      className={`flex-1 text-[11px] font-bold tracking-wide uppercase h-10 ${
                        userStatus === "visited"
                          ? "bg-brand-primary text-brand-primary-foreground border-brand-primary hover:opacity-90"
                          : ""
                      }`}
                    ...
                    >
                      <Bookmark
                        className={`w-3 h-3 mr-1 ${userStatus === "pending" ? "fill-current" : ""}`}
                      />
                      Save
                    </Button>
                    <Button
                  ...
                  {(userStatus === "visited" || userStatus === "pending") && (
                    <div
                      className="flex items-center gap-3 px-3 py-2.5 bg-surface-muted"
                  ...
                  {selectedCollectionIds.length > 0 && (
                    <Badge variant="outline" className="text-xs border-dashed">
                      In {selectedCollectionIds.length} collection
                      ...
                      {userImages.slice(0, 4).map((img) => {
                        ...
                        return (
                          <img
                            key={img.id}
                            src={publicUrl}
                            className="h-16 w-16 object-cover border bg-surface-muted cursor-pointer hover:opacity-90 transition-opacity flex-shrink-0"
                            alt="Your photo"
                  ...
                  <div className="flex flex-col gap-2 pt-1">
                    <Button
                      className="w-full bg-brand-primary text-brand-primary-foreground hover:opacity-90 text-[11px] font-bold tracking-wide uppercase h-10"
                      asChild
                    >
                      <Link
                        to={
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full text-[11px] font-bold tracking-wide uppercase h-9"
                  ...
                  {/* Collection selector (inline expander) */}
                  {showCollections && (
                  ...
                  {(accessSynthesis || building.access_notes) && (
                    <div className="mt-3 space-y-2">
                      ...
                      {building.access_notes && (
                        <div className="text-xs text-text-secondary border-l-2 border-brand-primary/30 pl-3 py-1 bg-surface-muted/30">
                          {building.access_notes}
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      {topLinks.map((link) => {
                        ...
                        return (
                          <div
                            key={link.link_id}
                            className="flex items-center justify-between px-4 py-2.5 border-b border-border-default last:border-b-0 hover:bg-surface-muted/50 transition-colors group"
                          >
                            <a
                              href={link.url}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="flex-1 flex flex-col gap-0.5 overflow-hidden cursor-pointer min-w-0"
                            >
                              <span className="font-medium truncate text-sm group-hover:text-brand-primary transition-colors">
                                {hasTitle ? link.title : displayDomain}
                              </span>
                              <div className="flex items-center gap-1.5 text-xs text-text-secondary">
                                {hasTitle && (
                                  <span className="truncate max-w-[110px]">{displayDomain}</span>
                                )}
                            ...
                            <div className="flex items-center gap-1 shrink-0 ml-2">
                              <Button
                                variant="ghost"
                                size="sm"
                                className={`h-7 px-2 gap-1 text-xs hover:bg-transparent ${
                                  isLiked
                                ...
                                aria-label={
                                  isLiked ? "Unlike resource" : "Like resource"
                                }
                              >
                                <Heart
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="p-1 rounded-sm hover:bg-surface-default text-text-secondary/50 hover:text-text-primary transition-colors"
                      ...
                      {showLinkEditor && user && (
                        <div className="p-4 border-t border-border-default space-y-2">
                          ...
                          <Input
                            ...
                            className="h-8 text-sm"
                          />
                          <Button size="sm" onClick={handleAddLink} className="w-full h-8">
                            Add Link
                          </Button>
                        </div>
                      )}
            ...
            {entries.length === 0 ? (
              ...
              <div className="border border-border-default">
                {entries.map((entry) => (
                  <div
                    key={entry.id}
                    className={`flex gap-4 px-5 py-5 border-b border-border-default last:border-b-0 hover:bg-surface-muted/30 transition-colors ${
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        {entry.user.is_verified_architect && (
                          <div
                            className="inline-flex items-center text-text-primary"
                        ...
                        {entry.status === "visited" && (
                          <Badge className="bg-brand-primary text-brand-primary-foreground text-[9px] h-4 px-1.5 font-bold tracking-wide uppercase">
                            Visited
                          </Badge>
                        )}
                        {entry.status === "pending" && (
                          <Badge
                            variant="secondary"
                            className="text-[9px] h-4 px-1.5 font-bold tracking-wide uppercase"
                          >
                            Saved
                          </Badge>
                        )}
                        ...
                        <Link
                          to={`/review/${entry.id}`}
                          className="text-xs text-text-disabled hover:underline ml-auto flex-shrink-0"
                        >
                          ...
                          {entry.images.map((img) => {
                            ...
                            return (
                              <img
                                key={img.id}
                                src={publicUrl}
                                className="h-20 w-20 object-cover border bg-surface-muted cursor-pointer hover:opacity-90 transition-opacity"
                                alt="Review photo"
                                onClick={() => setSelectedImage(displayImg)}
                              />
                            );
