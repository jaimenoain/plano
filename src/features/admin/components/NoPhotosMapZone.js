...
export const filterMissingPhotoBuildings = (buildings, publicReviewBuildingIds, parseLocationFn) => {
    return buildings
        .filter(b => {
        ...
        if (publicReviewBuildingIds.has(b.id))
            return false;
        return true;
    })
        .map(b => {
...
export function NoPhotosMapZone() {
    ...
    useEffect(() => {
        const fetchBuildings = async () => {
            try {
                ...
                if (reviewError)
                    throw reviewError;
                const buildingsWithPublicPhotos = new Set(publicReviews?.map(r => r.building_id).filter(Boolean));
    ...
    return (_jsxs(Card, { className: "col-span-4", children: [_jsx(CardHeader, { children: _jsxs(CardTitle, { children: ["Buildings Missing Photos (", buildings.length, ")"] }) }), _jsxs(CardContent, { className: "h-[500px] p-0 overflow-hidden rounded-b-lg relative", children: [loading && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center bg-surface-default/50 z-10", children: _jsx(Loader2, { className: "h-8 w-8 animate-spin" }) })), _jsxs(Map, { initialViewState: {
                            latitude: 20,
                            longitude: 0,
                            zoom: 1.5
                        }, style: { width: '100%', height: '100%' }, mapStyle: "https://tiles.openfreemap.org/styles/positron", mapLib: maplibregl, attributionControl: false, interactiveLayerIds: ['no-photos-circles'], onClick: handleMapClick, cursor: cursor, onMouseEnter: () => setCursor('pointer'), onMouseLeave: () => setCursor('auto'), children: [_jsx(NavigationControl, { position: "top-right" }), _jsx(Source, { type: "geojson", data: geoJsonData, children: _jsx(Layer, { ...CIRCLE_LAYER }) })] })] })] }));
}
