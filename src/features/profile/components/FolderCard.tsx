...
export function FolderCard({ folder, to, onClick, className, isDroppable = false }: FolderCardProps) {
  ...
  const content = (
    <>
      ...
      <div className="absolute inset-0 bg-surface-card border rounded-lg transform -rotate-2 translate-x-0.5 translate-y-1.5 transition-transform group-hover:-rotate-3 group-hover:translate-x-1 group-hover:translate-y-2 -z-20" />
      <div className="absolute inset-0 bg-surface-card border rounded-lg transform rotate-2 translate-x-1 translate-y-1 transition-transform group-hover:rotate-3 group-hover:translate-x-1.5 group-hover:translate-y-1.5 -z-10" />

      {/* Main Card */}
      <Card
        ref={setNodeRef}
        className={cn(
          "h-[100px] bg-surface-card hover:border-brand-primary/50 transition-all relative overflow-hidden shadow-none",
          isOver && "border-brand-primary ring-2 ring-brand-primary/40 bg-surface-muted/80 scale-105 z-20"
        )}
      >
        <CardContent className="p-3 h-full flex flex-col justify-between relative z-10 pointer-events-none">
          <div className="flex justify-between items-start gap-2">
             <h4 className="font-medium text-sm line-clamp-2 leading-tight group-hover:text-brand-primary transition-colors whitespace-normal flex-1">
               {folder.name}
             </h4>
             {folder.is_public ? (
          ...
          <div className="flex items-end justify-between mt-auto">
             ...
             {folder.preview_images && folder.preview_images.length > 0 ? (
               ...
               <div className="w-8 h-8 flex items-center justify-center bg-surface-muted/50 rounded text-text-secondary/50">
