export const handleDragEndLogic = async ({ activeId, overId, content, setContent, supabase, toast, setUpdatingItemId, }) => {
    if (!overId)
        return;
    if (activeId === overId)
        return;
    ...
    if (overId === "saved")
        newRating = 0;
    else if (overId === "1-point")
        newRating = 1;
    else if (overId === "2-points")
        newRating = 2;
    else if (overId === "3-points")
        newRating = 3;
    else {
    ...
    setContent((prev) => prev.map((item) => item.id === activeItem.id
        ? {
            ...item,
