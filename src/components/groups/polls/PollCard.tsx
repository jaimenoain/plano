...
export function PollCard({ poll, groupSlug, isAdmin }: PollCardProps) {
  ...
  const getStatusBadge = () => {
      ...
      if (isActive) return <Badge className="bg-white/20 text-primary-foreground hover:bg-white/30 border-none px-2 py-0.5 text-[10px]">Active</Badge>;

      if (isPublished) return <Badge variant="outline" className="border-primary-foreground/30 text-primary-foreground px-2 py-0.5 text-[10px]">Upcoming</Badge>;
  ...
  if (isLive) {
    ...
    borderStyles = "border-border/10";
     ...
     cardStyles = "bg-gradient-to-br from-primary/90 to-primary/80 text-primary-foreground border-transparent";
     contentStyles = "text-primary-foreground/90";
     borderStyles = "border-primary-foreground/20";
  }
