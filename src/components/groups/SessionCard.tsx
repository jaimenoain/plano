...
export function SessionCard({
  ...
  const handleAddToCalendar = () => {
    ...
    const url = createGoogleCalendarUrl({
      title: session.title || "Architecture Event",
      description,
      startTime: sDate,
    });
          ...
          <div>
            ...
            >
              <h3 className="text-xl font-bold leading-tight">
                {session.title || "Architecture Event"}
                ...
                >
                  <div className={`flex gap-5 p-5 ${showGroupStats ? "pb-2" : ""}`}>
                    <div className="relative shrink-0 w-20 h-28 rounded-md overflow-hidden border bg-muted shadow-sm">
                      <img src={getBuildingImageUrl(building.main_image_url) || '/placeholder.svg'} className="w-full h-full object-cover" alt={building.name} />
                    </div>
                      ...
                      <div className="mb-2 flex justify-between items-start">
                        ...
                        <div onClick={(e) => e.stopPropagation()}>
                          {myLog?.rating ? (
                            ...
                            <PersonalRatingButton buildingId={building.id} initialRating={null} status={myLog?.status} onRate={(bid, rating) => rateBuilding.mutate({ buildingId: bid, rating })} isLoading={rateBuilding.isPending} />
                          )}
                        </div>
                      </div>
