...
const InputOTPSlot = React.forwardRef(({ index, className, ...props }, ref) => {
    ...
    return (_jsxs("div", { ref: ref, className: cn("relative flex h-10 w-10 items-center justify-center border-y border-r border-border-default text-sm transition-all first:rounded-l-sm first:border-l last:rounded-r-sm", isActive && "z-10 ring-2 ring-brand-primary", className), ...props, children: [char, hasFakeCaret && (_jsx("div", { className: "pointer-events-none absolute inset-0 flex items-center justify-center", children: _jsx("div", { className: "animate-caret-blink h-4 w-px bg-text-primary duration-1000" }) }))] }));
});
