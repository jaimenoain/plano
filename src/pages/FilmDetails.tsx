...
export default function FilmDetails({ mediaType: propMediaType }: { mediaType?: "movie" | "tv" }) {
  ...
  // --- Data Fetching (Keep existing logic mostly) ---
  useEffect(() => {
    if (user) {
    ...
    try {
      ...
      if (existingFilm && hasExtendedDetails) {
        ...
        const filmToUpsert = {
            ...
            countries: tmdbData.production_countries?.map((c: any) => c.iso_3166_1) || [],
      ...
      if (filmUuid && user) {
        ...
        if (reviewData) {
            const formattedReviews: FeedReview[] = reviewData.map((r: any) => ({
                ...r,
  ...
  const handleRate = async (rating: number) => {
     // Legacy wrapper if needed, but ActionSection now calls handleSaveReview mostly or triggers local edit state
     // We will pass handleSaveReview to ActionSection instead of just handleRate
  };
  ...
  const { friendsText, friendsStars, communityText, communityStars, friendsWantToWatch } = useMemo(() => {
      ...
      // Sort ratings: prioritize those with tags
      const compareByTags = (a: FeedReview, b: FeedReview) => {
      ...
      return { friendsText: ft, friendsStars: fs, communityText: ct, communityStars: cs, friendsWantToWatch: fwtw };
    ...
    <AppLayout title="Details" showBack>
      ...
      <div className="w-full min-h-screen bg-background pb-20">
        ...
        <div className="lg:grid lg:grid-cols-[35%_65%] lg:gap-12 lg:max-w-7xl lg:mx-auto lg:px-8 lg:py-12">
            ...
            {/* === LEFT COLUMN: Reference & Identity (Sticky) === */}
            <div className="hidden lg:block space-y-6 h-fit sticky top-24">

                {/* Poster */}
                <div className="rounded-xl overflow-hidden shadow-2xl aspect-[2/3] border border-white/5 relative group">
...
function HeroSection({ movie, availability, availabilityLoading, onOpenAvailability, countryCode, subscribedPlatforms }: any) {
    const originalTitle = movie.original_title || movie.original_name;
    const localizedTitle = movie.title || movie.name;
    ...
    return (
        <div className="relative w-full overflow-hidden lg:rounded-2xl bg-black/40 lg:bg-transparent">
             ...
             <div className="relative z-10 p-4 pt-20 sm:pt-32 lg:p-0 flex flex-col items-center lg:items-start text-center lg:text-left">
                  ...
                  <div className="w-32 sm:w-48 lg:w-full lg:aspect-[2/3] rounded-lg shadow-2xl overflow-hidden mb-4 lg:mb-6 border border-white/10">
                      <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} className="w-full h-full object-cover" />
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3 text-sm text-muted-foreground mb-4">
                      ...
                      {genres?.length > 0 && (
                          <>
                             <span className="w-1 h-1 rounded-full bg-white/30" />
                             <span className="truncate max-w-[200px]">{genres.map((g: any) => g.name).join(", ")}</span>
                          </>
                      )}
                  </div>
...
function ActionSection({ userStatus, myRating, myReview, myTags, allTags, handleStatus, onSaveReview, handleRateNav, handleDeleteRating, showRecommend, watchWithInvitations = [], hasFriendRatings = false }: any) {
    ...
    return (
        <div className="w-full bg-card/90 backdrop-blur-sm p-4 shadow-sm z-20">
            {/* WATCHED STATE */}
            {userStatus === 'watched' && (
                <div className="space-y-4 group/section">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-1 group relative flex-wrap">
                            ...
                            <span className="ml-2 text-xl font-bold text-white">{hoverRating || myRating}</span>
                     ...
                     {watchWithInvitations.length > 0 && (
                        <div className="flex items-center gap-3 bg-muted/20 p-2 rounded-lg">
                            ...
                            <div className="flex -space-x-2">
                                {watchWithInvitations.map((p: any) => (
            ...
            <div className="pt-8 border-t border-white/5 space-y-4">
                <div className="flex flex-col gap-2 text-xs text-muted-foreground/60">
                     <div className="flex gap-2">
                         ...
                         <span>{movie.production_countries?.map((c: any) => c.name || c.iso_3166_1).join(", ")}</span>
                     </div>
                     <div className="flex gap-2">
                         <span className="font-semibold uppercase w-20 shrink-0">Languages</span>
                         <span>{movie.spoken_languages?.map((l: any) => l.english_name || l.iso_639_1).join(", ")}</span>
                     </div>
                     {movie.imdb_id && (
                         <div className="flex gap-2">
...
function CompactRatingRow({ review, community = false }: { review: FeedReview, community?: boolean }) {
    ...
    return (
        <div
            ...
            className={`
                flex items-start gap-3 p-2 rounded-lg hover:bg-muted/30 transition-colors cursor-pointer
        ...
        >
            ...
            <div className="flex items-center gap-2 flex-1 flex-wrap mt-0.5">
                <span className="text-sm font-medium truncate max-w-[120px]">{review.user.username}</span>
