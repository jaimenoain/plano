...
**Map markers are monochrome by default, and the rule is enforced.** Marker faces are set through inline `backgroundColor` because MapLibre portals markers outside the cascade that resolves our CSS custom properties, so every *system* marker colour is a literal. Those literals live in exactly one file — `src/features/maps/constants/mapMarkerFills.ts` — which mirrors the semantic tokens and carries a documented `eslint-disable`; the raw-hex guard covers the rest of `src/features/maps/**` as **`.ts` and `.tsx`**. Prominence is expressed by **ring weight and size, never by hue**. The lone exceptions are the photography-gap overlay, a data-coverage heatmap that keeps `--feedback-*` colours because it encodes a measurement rather than a place, and **collection maps** (see ADR 0033): a collection owner may set an arbitrary opaque hex colour and a named size per Categorization Method bucket, stored as data in `collections.marker_styles` and validated by `src/features/collections/markerStyles.ts` — never a literal in source, so the guard's rationale still holds. `/map`, `/search`, and every other map surface are unaffected and keep the monochrome ladder below. (This file once held `brandPrimary: "#BEFF00"` — written when `--brand-primary` *was* lime, never updated when the brand flipped to black. Keep it in sync, or the next redesign rots it again.)
