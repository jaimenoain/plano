...
|---|---|---|---|---|
| **Landing / Feed (dual-mode)** | `/` | [features/feed/pages/Index.tsx](../src/features/feed/pages/Index.tsx) | H | ⚠️ **One route, two experiences.** Logged-out → marketing landing (`LandingHero`, `LandingMarquee`, `LandingFeatureGrid`, `LandingNav`, `LandingFooter`). Logged-in → activity feed (`ReviewCardFeed`, `EditorialFeedPost`, `FeedActivitySummaryRow`, `FeedSidebar`; hooks `useHomeFeed`/`useCommunityFeed`). The rail's `MyLibraryModule` previews `/search?mode=library` as an ink-density plate (`LibraryAtlasPlate` + the pure `utils/libraryAtlas.ts`) — a hand-built SVG heatmap, deliberately not MapLibre, so the feed stays off the map bundle. Redesign should treat these as two distinct screens. |
...
| **Search** | `/search` | [features/search/SearchPage.tsx](../src/features/search/SearchPage.tsx) | H | Map + list + filters SERP. Rich components: `OmniSearchBar`, `DiscoverySearchInput`, `DiscoveryFiltersPanel`, `DiscoveryList`, `UserResultsList`, `SearchModeToggle`, `MapModeToggle` (always-visible **All / Discover / My map** switch — all three switch in place, `?mode=library` is a self-sufficient deep link and is no longer redirected to `/map`; see [ADR 0025](decisions/0025-my-map-as-mode-and-route.md)), `SmartFilterSuggestions`, `LeaderboardDialog`. Filter state is shared through `BuildingSearchProvider` (single `useBuildingSearch` instance). Recently rebuilt (pin-click → detail drawer). |
| **My Map** | `/map` | [features/mymap/MyMapPage.tsx](../src/features/mymap/MyMapPage.tsx) | H | The same surface as `/search`, pinned to library mode (`SearchPageShell forcedMode="library"`) so the mode never appears in the URL and the first cluster fetch is already filtered. The mode toggle shows with **My map** selected; its other segments navigate to `/search?mode=…` (`modeSwitchUrl`). `MyMapChrome` (stats band + "Start your map" first run) renders from library mode, so it appears on the `/search` tab too. Signed out → `/login?redirect=/map`. |
...
| **Collection map** | `/:username/map/:slug` | [features/collections/components/CollectionMapPage.tsx](../src/features/collections/components/CollectionMapPage.tsx) | H | Public/shared collection map (`CollectionMapGL`, `ItineraryList`, route planning). Client-side search (`?q=`, `useCollectionSearch`) narrows the All Items list and the map pins together. Owners/editors can switch on the discovery view (Settings → Map View) to overlay every building in the viewport and add from the map — see [ADR 0021](decisions/0021-collection-map-two-cluster-sources.md). While it is on, the rail gains a **Discover** tab listing the addable buildings in view, read from `get_buildings_list` rather than the cluster layer — see [ADR 0024](decisions/0024-collection-discover-rail-list.md). |
