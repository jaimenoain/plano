...
## [x] Phase 4 — Bigger bets (opened 2026-07-30 for 4.3 only, then closed)

Owner decision 2026-07-30: open Phase 4 with **4.3 alone**, then close the roadmap. 4.1 and
4.2 were never started — closed unstarted, not dropped on merit; re-open them by generating a
new roadmap. 4.3's own gate ("only if 2.2 measurably lifts photo contributions") was waived:
the owner opened it without waiting for that metric.
...
## Final UAT — run 2026-07-30

Every check below was run against **production**. The two that write were run inside a
transaction and rolled back, so nothing was fabricated in the live catalogue.

- **"A brand-new ambassador can find and complete a first contribution in one sitting
  without guidance, and the portal acknowledges it."** — **Partially verified.** The *find*
  half is live: as an active London ambassador, three of the five "Start here" queues return
  a full page of work (photo gaps 20, research 20, unclaimed firms 20; recent-buildings
  moderation is legitimately empty), and field mode offers 46 buildings within 10 km. The
  *acknowledge* half is live: the flag → `contribution_flagged` trigger fires (below), and
  `ambassador_milestones` holds real earned rows, with `first_contribution` awarded on the
  first contribution of any kind. **Not verified end-to-end:** the actual photo upload, which
  would write a real image into the production catalogue. Its path is unchanged since 2.2 and
  is covered by unit tests; the "it drops off the queue afterwards" step is guaranteed by the
  gap predicate corrected in 4.3a and checked directly against prod.
- **"Flagging bad content produces a row an admin actually sees."** — **Verified.** Flagging a
  real building as a real ambassador produced a `pending` row that the exact
  `/admin/moderation` query returns (`content_type = 'building'`, reporter resolved), and the
  `trg_notify_contribution_flagged` trigger delivered a `contribution_flagged` notification to
  the building's original contributor — not to the reporter. Rolled back.
- **"An ambassador can see their own impact (not just leaders), and inactive members receive a
  weekly nudge they can switch off."** — **Verified.** `/embassy/impact` is in the tab bar for
  every ambassador with no leader gate, and both `get_my_ambassador_impact` and
  `sync_my_ambassador_milestones` are self-scoped on `auth.uid()`. The `embassy-weekly-digest`
  cron job is active on `0 9 * * 1` with a delivered, notified row for week 2026-07-20. The
  off switch works on **both** channels: with `notification_preferences.weekly_digest =
  "false"`, an attempted in-app insert wrote 0 rows (the `before_insert_notifications` trigger
  drops it) and `get_pending_weekly_digest_emails` queued 0 emails. Rolled back.

## Delivery Record

**Closed 2026-07-30** by owner decision, after 4.3. Installed 2026-07-23; 7 days end to end.

**What shipped.** Phases 0–3 complete, Phase 4 opened for one bet.
Phase 0 — owner prerequisites closed (#1635). Phase 1 — five trust fixes: the
onboarding↔Contribute tool-preference contract, real flagging into the admin queue, campaign
outreach progress, one front door at `/embassy` (729 dead lines deleted), and visible failure
states for event discovery (#1626–#1631). Phase 2 — the productive session: a "Start here"
queue of up to five ready tasks, in-tool photo upload, contribution-outcome notifications, and
one-click suggested goals with four new counting metrics (#1633, #1636, #1637, migrations
20271183/20271184). Phase 3 — the return loop: a personal `/embassy/impact` page with a
server-computed weekly streak, a Monday-morning weekly digest (in-app + email, opt-out, backed
by a delivery ledger and pg_cron), and four earnable milestones announced once each
(#1676, migrations 20271185/20271193/20271195). Phase 4 — field mode: `/embassy/field`, the
nearest photo gaps with the camera one tap away (#1677, migration 20271197).

**What was descoped.** 4.1 (pre-publish moderation for new buildings) and 4.2 (missions)
were never opened — owner decision 2026-07-30, closed unstarted rather than dropped on
merit. 4.3's own gate ("only if 2.2 measurably lifts photo contributions") was waived: it was
opened without waiting for that metric.

**Three bugs the work surfaced, all pre-existing, all fixed here.** Each had been invisible
because it failed silently rather than loudly:
1. **Moderation counted zero everywhere** (3.3a, migration 20271196) — the metric tested
   `building_audit_logs.table_name` for markers that are written to `operation`. 0 rows
   matched against 1508; every approval had been counted as a plain edit since the metric
   existed. Fixed in all three functions that carry the predicate at once.
2. **The photo-gap queue never excluded anything** (4.3a, migration 20271197) —
   `review_images.review_id` is a FK to `building_posts.id`, but the predicate joined it to
   `user_buildings.id`. It matched only because 18009 of 18021 legacy rows share a uuid, and
   was blind to every post written by current code.
3. **`REVOKE … FROM PUBLIC` never locked an RPC down** (found while shipping 3.2) — an
   anonymous `rpc/run_weekly_digest` returned 200 and ran. Swept separately in #1672 across 33
   functions, and now a standing rule in `AGENTS.md`.

**Specs.** [`docs/specs/embassy-ambassador-experience.md`](specs/embassy-ambassador-experience.md)
remains the design rationale and metrics reference; nothing in it moved at close-out.
